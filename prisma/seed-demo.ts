...
const LEGAL_REFERENCES = [
  ...
  },
  {
    ...
    articles: [
      {
        articleNumber: '第十条',
        content: '著作权包括下列人身权和财产权：（一）发表权；（二）署名权；（三）修改权；（四）保护作品完整权；（五）复制权；（六）发行权；（七）出租权；（八）展览权；（九）表演权；（十）放映权；（十一）广播权；（十二）信息网络传播权；（十三）摄制权；（十四）改编权；（十五）翻译权；（十六）汇编权。',
      },
      {
        articleNumber: '第四十七条',
        content: '有下列侵权行为的，应当根据情况，承担停止侵害、消除影响、赔礼道歉、赔偿损失等民事责任：（一）未经著作权人许可，复制、发行、表演、放映、广播、汇编、通过信息网络向公众传播其作品的。',
      },
    ],
  },
  {
    ...
    articles: [
      {
        articleNumber: '第五十七条',
        content: '有下列行为之一的，均属侵犯注册商标专用权：（一）未经商标注册人的许可，在同一种商品上使用与其注册商标相同的商标的；（二）未经商标注册人的许可，在同一种商品上使用与其注册商标近似的商标，或者在类似商品上使用与其注册商标相同或者近似的商标，容易导致混淆的。',
      },
    ],
  },
  {
    ...
    articles: [
      {
        articleNumber: '第二条',
        content: '经营者在生产经营活动中，应当遵循自愿、平等、公平、诚信的原则，遵守法律和商业道德。',
      },
      {
        articleNumber: '第十二条',
...
async function main() {
  ...
  try {
    ...
    // 创建所有案件
    const allCases = [];
    for (const category of [...CASE_TEMPLATES.required, ...CASE_TEMPLATES.optional]) {
    ...
    for (const caseData of allCases) {
      ...
      for (let roundNum = 1; roundNum <= 2; roundNum++) {
        const round = await prisma.debateRound.create({
          data: {
            ...
            status: roundNum === 1 ? RoundStatus.COMPLETED : RoundStatus.IN_PROGRESS,
            startedAt: new Date(Date.now() - (2 - roundNum) * 24 * 60 * 60 * 1000),
            completedAt: roundNum === 1 ? new Date(Date.now() - 24 * 60 * 60 * 1000) : null,
          },
        });
        stats.rounds++;
        ...
        for (const arg of argumentData) {
          ...
          await prisma.aIInteraction.create({
            data: {
              ...
              model: argument.aiProvider === 'deepseek' ? 'deepseek-chat' : 'glm-4-flash',
              request: {
      ...
      for (const ref of selectedReferences) {
        const article = ref.articles[Math.floor(Math.random() * ref.articles.length)];
