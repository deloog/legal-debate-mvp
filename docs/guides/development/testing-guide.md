...
如果本次改动涉及 API 路由、认证、权限或审计日志，建议同时查看：

- [API 测试审计 Mock 规范](../../../development/testing/API_TEST_AUDIT_MOCK_GUIDELINES.md)

该规范说明了：

- `src/__tests__/api/setup.js` 中公共 mock 的职责
- 什么时候应复用公共审计日志 mock
- 什么时候应在测试文件中显式覆盖审计日志 mock
- 推荐的 API 测试错误断言写法
