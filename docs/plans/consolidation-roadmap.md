...
| #   | 任务                                    | 说明                                             | 状态      |
| --- | --------------------------------------- | ------------------------------------------------ | --------- |
| M-1 | 将 `strict-migration` 分支合并到 `main` | 第一、二阶段完成后执行，标志着严格迁移的正式收官 | ✅ 已完成 |
| M-2 | 打标签 `v2.0-stable`                    | 记录一个稳定版本基线，方便后续回滚               | ✅ 已完成 |

> ✅ **子阶段 3A 完成（2026-02-28）**：strict-migration 分支已成功合并到 main，并打标签 v2.0-stable

### 子阶段 3B：已知 UX 问题修复

| #   | 任务                                                                 | 涉及模块                                                                   | 用户价值 | 状态      |
| --- | -------------------------------------------------------------------- | -------------------------------------------------------------------------- | -------- | --------- |
| U-1 | 审计合同审批流程的前端交互（基于 P0-9 新实现）                       | `WorkflowDesigner`、`ApprovalAnalyticsDashboard`                           | ⭐⭐⭐   | ⏳ 待处理 |
| U-2 | 验证 AI 分析结果的展示组件（`AIAssessmentBadge` 等）在各页面正确显示 | `consultation/AIAssessmentCard` 等                                         | ⭐⭐⭐   | ⏳ 待处理 |
| U-3 | 检查合规检查（Compliance）页面与后端服务的数据联通情况               | `app/compliance/`、`lib/enterprise/compliance-service`                     | ⭐⭐     | ⏳ 待处理 |
| U-4 | 验证通知提醒（ReminderList）在合同到期前能正确触发                   | `components/reminder/`、`lib/contract/contract-milestone-reminder-service` | ⭐⭐     | ⏳ 待处理 |

> ⚠️ **子阶段 3B 状态**：由于合并后测试基础设施存在问题（数据库迁移未完成），UX 问题修复暂未开始

### 子阶段 3C：技术性改进（按需执行）

| #   | 任务                                                                         | 说明             | 优先级 | 状态      |
| --- | ---------------------------------------------------------------------------- | ---------------- | ------ | --------- |
| D-1 | 补全 `src/components/consultation/AIAssessmentCard.tsx` 的类型（当前有修改） | 小型类型修复     | 🟡 中  | ⏳ 待处理 |
| D-2 | 检查 `src/types/` 下各类型文件是否与当前 Prisma Schema 同步                  | 防止类型漂移     | 🟡 中  | ⏳ 待处理 |
| D-3 | 清理 `src/__tests__/integration/` 目录中的空目录残留                         | 保持代码仓库整洁 | 🟢 低  | ⏳ 待处理 |

> ⚠️ **测试问题说明**：合并后部分测试失败，主要原因：
>
> 1. 数据库迁移 `20240224_add_knowledge_graph_quality_score` 未完成
> 2. 测试使用 `testPrisma` 直接访问数据库，schema 不匹配
> 3. 建议：运行 `npx prisma migrate dev` 解决迁移问题后，再进行 UX 修复
