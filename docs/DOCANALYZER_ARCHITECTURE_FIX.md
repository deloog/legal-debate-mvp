...
class DocAnalyzerAgent {
  ...
  private async cacheLayer(input: DocumentAnalysisInput, output?: DocumentAnalysisOutput) {
    // 检查缓存
    // 写入缓存
  }
}
...
export class FilterProcessor {
  ...
  private validateFormat(text: string, docType: DocumentType): FormatCheckResult {
    // 检查必要的字段是否存在
    // 检查格式是否正确
  }
}
  ...
  async process(data: ExtractedData, text: string): Promise<RuleProcessResult> {
    ...
    for (const claim of data.claims) {
      if (!claim.amount && claim.content) {
        const amountResult = await this.amountExtractor.extractFromText(claim.content);
