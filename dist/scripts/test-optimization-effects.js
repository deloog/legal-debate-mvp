...
const doc_analyzer_optimized_1 = require("../src/lib/agent/doc-analyzer-optimized");
const agent_1 = require("../src/types/agent");
const lawsuit_request_classifier_1 = require("../src/lib/classification/lawsuit-request-classifier");
const amount_extractor_precision_1 = require("../src/lib/extraction/amount-extractor-precision");
const fs_1 = require("fs");
const path_1 = require("path");
class OptimizationTester {
    constructor() {
        this.analyzer = new doc_analyzer_optimized_1.DocAnalyzerAgentOptimized();
        this.claimClassifier = new lawsuit_request_classifier_1.LawsuitRequestClassifier();
        this.amountExtractor = new amount_extractor_precision_1.PrecisionAmountExtractor();
    }
    /**
     * 运行优化效果测试
     */
    ...
    loadTestDocuments() {
        ...
        try {
            // 测试文档1：复杂的民事纠纷
            const civilCasePath = (0, path_1.join)(process.cwd(), 'test-data/legal-documents/test-variation-civil-case.txt');
            const civilCaseContent = (0, fs_1.readFileSync)(civilCasePath, 'utf-8');
            testDocs.push({
                id: 'civil-case-001',
                name: '复杂民事货款纠纷案',
                ...
                expectedClaims: {
                    types: ['PAY_PRINCIPAL', 'PAY_INTEREST', 'PAY_PENALTY', 'LITIGATION_COST'],
        ...
        `,
                ...
                expectedClaims: {
                    types: ['PAY_DAMAGES', 'PAY_PENALTY', 'PERFORMANCE', 'LITIGATION_COST'],
    ...
    async testSingleDocument(doc) {
        ...
        try {
            ...
            const context = {
                ...
                priority: agent_1.TaskPriority.MEDIUM,
                data: {
                    documentId: doc.id,
                    filePath: '',
                    fileType: 'TXT',
            ...
            if (doc.expectedParties) {
                result.partyAccuracy = this.evaluatePartyAccuracy(analysis.extractedData.parties, doc.expectedParties, result);
            }
            // 评估诉讼请求准确率
            if (doc.expectedClaims) {
                result.claimAccuracy = this.evaluateClaimAccuracy(analysis.extractedData.claims, doc.expectedClaims, result);
            }
            // 评估金额识别精度
            if (doc.expectedAmounts) {
                result.amountAccuracy = this.evaluateAmountAccuracy(analysis.extractedData.claims, doc.expectedAmounts, result);
            }
            // 计算整体准确率
            result.overallAccuracy = (result.partyAccuracy + result.claimAccuracy + result.amountAccuracy) / 3;
            // 记录改进效果
            ...
            result.errors.push(`分析过程中发生错误: ${error.message}`);
        }
     ...
     * 评估当事人信息准确率
     */
    evaluatePartyAccuracy(actualParties, expectedParties, result) {
        let correctCount = 0;
        let totalCount = 0;
        // 检查原告
        if (expectedParties.plaintiffs) {
            totalCount += expectedParties.plaintiffs.length;
            const actualPlaintiffs = actualParties.filter((p) => p.type === 'plaintiff').map((p) => p.name);
            correctCount += this.calculateMatchCount(actualPlaintiffs, expectedParties.plaintiffs);
        }
        // 检查被告
        if (expectedParties.defendants) {
            totalCount += expectedParties.defendants.length;
            const actualDefendants = actualParties.filter((p) => p.type === 'defendant').map((p) => p.name);
            correctCount += this.calculateMatchCount(actualDefendants, expectedParties.defendants);
        }
        // 检查法定代表人
        if (expectedParties.legalRepresentatives) {
            totalCount += expectedParties.legalRepresentatives.length;
            const actualLegalReps = actualParties.filter((p) => p.type === 'other' && p.role && p.role.includes('法定代表人')).map((p) => p.name);
            correctCount += this.calculateMatchCount(actualLegalReps, expectedParties.legalRepresentatives);
        }
     ...
     * 评估诉讼请求准确率
     */
    evaluateClaimAccuracy(actualClaims, expectedClaims, result) {
        let correctCount = 0;
        let totalCount = expectedClaims.types.length;
        ...
        if (expectedClaims.amounts) {
            ...
            for (let i = 0; i < Math.min(actualAmounts.length, expectedClaims.amounts.length); i++) {
                const diff = Math.abs(actualAmounts[i] - expectedClaims.amounts[i]);
                if (expectedClaims.amounts[i] > 0 && diff / expectedClaims.amounts[i] < 0.01) { // 允许1%误差
                    correctCount += 0.5; // 金额匹配给部分分数
                }
                else if (expectedClaims.amounts[i] !== 0) {
                    result.warnings.push(`金额不匹配: 期望 ${expectedClaims.amounts[i]}, 实际 ${actualAmounts[i]}`);
                }
            }
        }
        return totalCount > 0 ? Math.min((correctCount / totalCount) * 100, 100) : 0;
    }
    /**
     * 评估金额识别精度
     */
    evaluateAmountAccuracy(actualClaims, expectedAmounts, result) {
        const actualAmounts = actualClaims
            .filter((claim) => claim.amount)
            .map((claim) => claim.amount);
        ...
        if (actualAmounts.length !== expectedAmounts.length) {
            result.warnings.push(`金额数量不匹配: 期望 ${expectedAmounts.length}, 实际 ${actualAmounts.length}`);
            return 0;
        }
        let correctCount = 0;
        for (let i = 0; i < actualAmounts.length; i++) {
            const diff = Math.abs(actualAmounts[i] - expectedAmounts[i]);
            if (expectedAmounts[i] > 0 && diff / expectedAmounts[i] < 0.01) { // 允许1%误差
                correctCount++;
            }
            else {
                result.warnings.push(`金额识别不准确: 期望 ${expectedAmounts[i]}, 实际 ${actualAmounts[i]}`);
            }
        }
     ...
     * 计算匹配数量
     */
    calculateMatchCount(actual, expected) {
        let count = 0;
        for (const expectedItem of expected) {
            if (actual.some(actualItem => actualItem.includes(expectedItem) || expectedItem.includes(actualItem))) {
                count++;
            }
        }
        return count;
    ...
    generateOptimizationReport(results) {
        ...
        const avgPartyAcc = results.reduce((sum, r) => sum + r.partyAccuracy, 0) / totalDocs;
        const avgClaimAcc = results.reduce((sum, r) => sum + r.claimAccuracy, 0) / totalDocs;
        const avgAmountAcc = results.reduce((sum, r) => sum + r.amountAccuracy, 0) / totalDocs;
        const avgOverallAcc = results.reduce((sum, r) => sum + r.overallAccuracy, 0) / totalDocs;
        // 基于之前的80%基准计算改进
    ...
    generateRecommendations(results) {
        ...
        const avgParty = results.reduce((sum, r) => sum + r.partyAccuracy, 0) / results.length;
        const avgClaim = results.reduce((sum, r) => sum + r.claimAccuracy, 0) / results.length;
        const avgAmount = results.reduce((sum, r) => sum + r.amountAccuracy, 0) / results.length;
        if (avgParty < 98) {
            recommendations.push('当事人信息识别需要进一步优化，建议增加更多角色识别规则');
        }
        if (avgClaim < 95) {
    ...
    displayOptimizationReport(report) {
        ...
        console.log('\n🎯 目标达成情况:');
        console.log(`当事人信息准确率≥98%: ${report.averagePartyAccuracy >= 98 ? '✅ 已达成' : '❌ 未达成'}`);
        console.log(`诉讼请求准确率≥95%: ${report.averageClaimAccuracy >= 95 ? '✅ 已达成' : '❌ 未达成'}`);
        console.log(`金额识别精度≥99%: ${report.averageAmountAccuracy >= 99 ? '✅ 已达成' : '❌ 未达成'}`);
...
async function main() {
    ...
    try {
        ...
        // 保存报告到文件
        const reportPath = (0, path_1.join)(__dirname, '../optimization-effect-report.json');
