...
class ConnectionPoolStressTester {
    ...
    async testConcurrentConnections(config) {
        console.log(`🔄 开始并发连接测试 (并发数: ${config.maxConcurrency}, 操作数: ${config.totalOperations})`);
        ...
        for (let i = 0; i < config.totalOperations; i++) {
            ...
            const promise = connection_manager_1.connectionManager.executeWithRetry(async (connection) => {
            ...
            if (config.operationDelay > 0) {
                await new Promise(resolve => setTimeout(resolve, config.operationDelay));
            }
        }
    ...
    async testPoolStress(config) {
        ...
        while (Date.now() < endTime) {
            const operationStart = Date.now();
            try {
                await connection_manager_1.connectionManager.executeWithRetry(async (connection) => {
    ...
    // 3. 连接泄漏检测测试
    async testConnectionLeaks(config) {
        console.log(`🔍 开始连接泄漏检测测试`);
        const startTime = Date.now();
        const initialStats = connection_manager_1.connectionManager.getStats();
        const errors = [];
        let operations = 0;
        // 故意不释放一些连接来测试泄漏检测
        const connections = [];
        for (let i = 0; i < config.maxConcurrency; i++) {
            try {
                const connection = await connection_manager_1.connectionManager.getConnection();
        ...
        for (let i = 0; i < halfLength; i++) {
            await connection_manager_1.connectionManager.releaseConnection(connections.pop());
        }
        // 再等待一段时间
        await new Promise(resolve => setTimeout(resolve, 1000));
        // 释放剩余连接
        ...
        console.log(`  连接泄漏: ${finalStats.activeConnections > 0 ? '❌ 检测到泄漏' : '✅ 无泄漏'}`);
        return result;
    }
    // 4. 超时和重试机制测试
    async testTimeoutAndRetry() {
        ...
        let operations = 0;
        // 测试短超时配置
        const shortTimeoutManager = new connection_manager_1.ConnectionManager({
        ...
        for (let i = 0; i < 2; i++) {
            try {
                ...
                errors.push(`连接获取失败: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        // 尝试获取更多连接（应该超时）
    ...
    async testHealthCheck() {
        ...
        for (let i = 0; i < 20; i++) {
            const operationStart = Date.now();
            try {
                const isHealthy = await (0, connection_pool_1.checkPoolHealth)();
                const responseTime = Date.now() - operationStart;
                responseTimes.push(responseTime);
                operations++;
                if (!isHealthy) {
                ...
                errors.push(`健康检查异常 #${i}: ${error instanceof Error ? error.message : String(error)}`);
            }
            // 间隔检查
    ...
    generateReport() {
        ...
        const avgResponseTime = this.results.reduce((sum, r) => sum + r.averageResponseTime, 0) / totalTests;
        console.log('\n🎯 总体统计:');
        ...
        this.results.forEach((result, index) => {
            ...
            console.log(`     吞吐量: ${result.throughput.toFixed(2)} ops/sec, 响应时间: ${result.averageResponseTime.toFixed(2)}ms`);
        });
        console.log('\n🏆 性能评估:');
