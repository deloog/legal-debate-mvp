...
class LawsuitRequestClassifier {
     ...
     * 分类诉讼请求
     */
    async classifyLawsuitRequests(requestText) {
        const claims = [];
        const warnings = [];
        let totalAmount = 0;
     ...
     * 分类单个请求
     */
    async classifySingleClaim(claimText) {
        const matchedTypes = [];
        // 遍历所有分类规则
     ...
     * 验证分类结果
     */
    validateClassification(claims, originalText) {
...
exports.LawsuitRequestClassifier = LawsuitRequestClassifier;
