...
class LawStarClient {
    ...
    async executeRegulationRequest(request) {
        ...
        if (request.lawType)
            params.append("effective", request.lawType);
        if (request.effectLevel)
            params.append("lawlevel", request.effectLevel);
        if (request.validity)
            params.append("timelinessnew", request.validity);
        if (request.startDate)
            params.append("startDate", request.startDate);
        if (request.endDate)
            params.append("endDate", request.endDate);
        if (request.depName)
            params.append("depName", request.depName);
        if (request.fileNum)
            params.append("fileNum", request.fileNum);
        return this.makeRequest(`${url}?${params.toString()}`, authToken, config.timeout || 30000);
    }
    // =============================================================================
    // 向量查询接口
    // =============================================================================
    ...
    async executeVectorRequest(request) {
        ...
        if (request.query)
            params.append("vector", request.query);
        if (request.keyword)
            params.append("keyword", request.keyword);
        if (request.lawType)
            params.append("xls", request.lawType);
        if (request.areaFacet)
            params.append("areaFacet", request.areaFacet);
        if (request.fbdwFacet)
            params.append("fbdwFacet", request.fbdwFacet);
        if (request.topicsFacet)
            params.append("topicsFacet", request.topicsFacet);
        if (request.rawnumber)
            params.append("rawnumber", request.rawnumber);
        if (request.filenum)
            params.append("filenum", request.filenum);
