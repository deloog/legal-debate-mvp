...
class AIMonitor {
    ...
    recordResponse(requestId, provider, model, success, responseTime, tokensUsed = 0, cacheHit = false, errorType) {
        const metrics = {
            provider,
            model,
            timestamp: Date.now(),
    ...
    countRateLimitHits(provider, timeWindow) {
        const cutoffTime = Date.now() - timeWindow;
        return this.events.filter((e) => e.type === "rate_limit_hit" &&
            e.provider === provider &&
            e.timestamp > cutoffTime).length;
    }
    // =============================================================================
    // 状态查询方法
    // =============================================================================
    getServiceStatus() {
        const totalRequests = Array.from(this.metrics.values()).reduce((sum, metricsList) => sum + metricsList.length, 0);
        const totalErrors = Array.from(this.metrics.values()).reduce((sum, metricsList) => sum + metricsList.filter((m) => !m.success).length, 0);
        const providerStatus = {};
        this.healthStatus.forEach((health, provider) => {
            providerStatus[provider] = health;
        });
        const averageResponseTime = this.calculateAverageResponseTime(Array.from(this.metrics.values()).flat());
        return {
            initialized: true,
