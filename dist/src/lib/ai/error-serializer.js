...
// 错误序列化器类
// =============================================================================
class AIErrorSerializer {
    /**
     * 序列化AI错误对象
     */
    static serialize(error, context, config) {
        ...
        const serialized = {
            ...
            message: this.truncateMessage(standardError.message, finalConfig.maxMessageLength),
     ...
     * 序列化为JSON字符串
     */
    static serializeToJson(error, context, config) {
    ...
    static deserializeFromJson(jsonString) {
        try {
            ...
            if (parsed.version !== this.VERSION) {
                console.warn(`Error serialization version mismatch: expected ${this.VERSION}, got ${parsed.version}`);
            }
            return parsed;
        }
        catch (error) {
            throw new Error(`Failed to deserialize error JSON: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 创建用户友好的错误消息
    ...
    static generateSummary(errors) {
        ...
        const mostCommonError = Object.entries(errorTypes).sort(([, a], [, b]) => b - a)[0]?.[0] ||
            "unknown_error";
        // 找出问题最多的提供商
        const mostProblematicProvider = Object.entries(providers).sort(([, a], [, b]) => b - a)[0]?.[0] ||
            "zhipu";
        return {
     ...
     * 提取错误详情
     */
    static extractErrorDetails(error, config) {
        const details = {};
        // 添加堆栈跟踪
...
// 默认导出
// =============================================================================
exports.default = AIErrorSerializer;
