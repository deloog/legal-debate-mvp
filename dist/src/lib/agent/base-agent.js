...
class BaseAgent {
     ...
     */
    async cleanup() {
        this.log(types_1.AgentLogLevel.INFO, 'Agent cleaned up', { name: this.name });
        this.status = agent_1.AgentStatus.IDLE;
    }
    /**
     * 健康检查
    ...
    async execute(context) {
        ...
        try {
            ...
            if (!validation.valid) {
                const error = (0, types_1.createAgentError)('INVALID_CONTEXT', `Invalid agent context: ${validation.errors.join(', ')}`, agent_1.AgentErrorType.VALIDATION_ERROR, this.name, false);
    ...
    createErrorFromException(error) {
        if (error && typeof error === 'object' && 'code' in error) {
            return (0, types_1.createAgentError)(error.code, error.message || 'Unknown error', this.mapErrorType(error.code), this.name, this.isRetryableError(error.code), { originalError: error });
        }
        return (0, types_1.createAgentError)('EXECUTION_ERROR', error?.message || 'Unknown execution error', agent_1.AgentErrorType.EXECUTION_ERROR, this.name, true, { originalError: error });
    }
    /**
     * 映射错误类型
     */
