...
class PrecisionAmountExtractor {
    constructor() {
        ...
        this.chineseNumberMap = {
            ...
            '万': 10000, '萬': 10000,
            '亿': 100000000, '億': 100000000, '乙': 100000000
        };
        this.currencyMap = {
            '元': 'CNY', '圆': 'CNY', '人民币': 'CNY',
            '￥': 'CNY', 'CNY': 'CNY', 'RMB': 'CNY',
    ...
    validateAndNormalize(match) {
        ...
        if (match.normalizedAmount > 1000000000) { // 超过1000万
    ...
    isInLegalContext(context) {
        const legalKeywords = [
            '诉讼', '请求', '判令', '支付', '偿还', '赔偿', '违约', '利息',
            '本金', '费用', '损失', '合同', '义务', '责任', '金额'
        ];
    ...
    validateAmountConsistency(amounts) {
        ...
        if (currencies.size > 1) {
            inconsistencies.push(`多种货币单位: ${Array.from(currencies).join(', ')}`);
    ...
    calculateRiskLevel(inconsistencies, totalAmounts) {
        const inconsistencyRatio = inconsistencies.length / Math.max(totalAmounts, 1);
        if (inconsistencyRatio === 0)
            return 'low';
    ...
    getBestExtraction(results) {
        ...
        const sortedResults = results.sort((a, b) => {
            ...
            // 置信度相同时，选择更精确的提取方法
            if (a.extractionMethod === 'ai_confirmed' && b.extractionMethod !== 'ai_confirmed') {
                return -1;
            }
            if (b.extractionMethod === 'ai_confirmed' && a.extractionMethod !== 'ai_confirmed') {
                return 1;
            }
