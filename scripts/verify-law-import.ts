...
async function verifyImport() {
  ...
  try {
    ...
    for (const article of sampleArticles) {
      console.log(`  ✓ ${article.lawName} ${article.articleNumber} (${article.category})`);
    }
    console.log('');
    ...
    console.log(`  完整率: ${((completeCount / totalCount) * 100).toFixed(1)}%`);

    if (incompleteFields.length > 0) {
      console.log(`  ⚠️  缺失字段: ${[...new Set(incompleteFields)].join(', ')}`);
    }
    console.log('');
