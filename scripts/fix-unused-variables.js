...
for (const [filePath, fileErrors] of errorsByFile.entries()) {
  ...
  for (const error of fileErrors) {
    ...
    if (line.includes(` ${variable}:`) || line.includes(`(${variable},`) || line.includes(`(${variable})`)) {
      // 在参数定义中添加下划线前缀
    ...
    // 模式4: private/public方法 - private methodName( -> private _methodName(
    if (!fixed && (line.includes('private ') || line.includes('public ')) && line.includes(`${variable}(`)) {
      line = line.replace(
        new RegExp(`(private|public)\\s+${variable}\\(`),
        `$1 _${variable}(`
      );
      fixed = true;
    }

    // 模式5: 常量定义 - const NAME = -> const _NAME = (readonly常量)
    if (!fixed && line.includes('readonly') && line.includes(` ${variable} =`)) {
      line = line.replace(` ${variable} =`, ` _${variable} =`);
      fixed = true;
    }

    // 模式6: for循环解构 - for (const [key, value] of ...
    if (!fixed && line.includes(` [${variable}`)) {
      line = line.replace(` [${variable}`, ` [_${variable}`);
      fixed = true;
    }

    // 模式7: 带类型的常量 - const NAME: Type = value
    if (!fixed && line.includes(`const ${variable}:`)) {
      line = line.replace(`const ${variable}:`, `const _${variable}:`);
      fixed = true;
    }
