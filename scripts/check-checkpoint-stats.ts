...
const checkpointPath = path.join(process.cwd(), 'data', 'crawled', 'flk', 'checkpoint.json');
...
try {
  ...
  // 检查是否有未完成的类型
  console.log('\n各类型下载进度:');
  Object.entries(checkpoint.types || {}).forEach(([type, info]: [string, any]) => {
    const status = info.completed ? '✅ 完成' : `⏳ ${info.page}/${info.totalPages}页`;
