...
class OptimizationTester {
  ...
  private loadTestDocuments(): TestDocument[] {
    ...
    try {
      // 测试文档1：复杂的民事纠纷
      const civilCasePath = join(process.cwd(), 'test-data/legal-documents/test-variation-civil-case.txt');
   ...
   * 评估当事人信息准确率
   */
  private evaluatePartyAccuracy(actualParties: any, expectedParties: any, result: TestResult): number {
    let correctCount = 0;
    let totalCount = 0;
    ...
    if (expectedParties.plaintiffs) {
      ...
      const actualPlaintiffs = actualParties.filter((p: any) => p.type === 'plaintiff').map((p: any) => p.name);
    ...
    if (expectedParties.defendants) {
      ...
      const actualDefendants = actualParties.filter((p: any) => p.type === 'defendant').map((p: any) => p.name);
    ...
    if (expectedParties.legalRepresentatives) {
      ...
      const actualLegalReps = actualParties.filter((p: any) => p.type === 'other' && p.role && p.role.includes('法定代表人')).map((p: any) => p.name);
   ...
   * 评估诉讼请求准确率
   */
  private evaluateClaimAccuracy(actualClaims: any, expectedClaims: any, result: TestResult): number {
    let correctCount = 0;
   ...
   * 评估金额识别精度
   */
  private evaluateAmountAccuracy(actualClaims: any, expectedAmounts: number[], result: TestResult): number {
    const actualAmounts = actualClaims
      .filter((claim: any) => claim.amount)
      .map((claim: any) => claim.amount);
   ...
   * 生成优化报告
   */
  private generateOptimizationReport(results: TestResult[]): OptimizationReport {
