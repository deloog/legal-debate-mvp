...
async function main() {
  ...
  try {
    ...
    const casesWithDetails = await Promise.all(
      cases.map(async (caseItem) => {
        ...
        const debateArguments = await Promise.all(
          (await prisma.debate.findMany({ where: { caseId: caseItem.id } })).map(
            async (debate) => {
              const argumentCount = await prisma.argument.count({
                where: { roundId: { in: (await prisma.debateRound.findMany({ where: { debateId: debate.id } })).map(r => r.id) } }
              });
        ...
        const totalArguments = debateArguments.reduce((sum: number, count: number) => sum + count, 0);
    ...
    // 获取文档、辩论、论点、法条数据
    const [documents, debates, argumentCount, legalReferences] = await Promise.all([
    ...
    result.summary.cacheHitRate = Number((totalHits / (totalRefs * 2)).toFixed(4)); // 假设平均每个法条被查询2次
...
async function performValidationChecks(result: ValidationResult) {
  ...
  if (result.summary.totalCases === 15) {
    ...
    checks.failedChecks.push(`❌ 案件数量错误，期望15个，实际${result.summary.totalCases}个`);
    checks.criticalIssues.push('案件数量不符合预期');
  }

  // 检查3: 每个案件类型覆盖
  const caseTypes = new Set(result.details.cases.map(c => c.type));
  const expectedTypes = ['INTELLECTUAL', 'COMMERCIAL', 'CIVIL', 'LABOR', 'CRIMINAL', 'ADMINISTRATIVE', 'OTHER'];
  ...
  // 检查4: 每个案件必须有文档
  const casesWithoutDocuments = result.details.cases.filter(c => !c.hasDocuments);
  if (casesWithoutDocuments.length === 0) {
    checks.passedChecks.push('✅ 所有案件都有文档');
  } else {
    checks.failedChecks.push(`❌ ${casesWithoutDocuments.length}个案件缺少文档`);
  ...
  // 检查6: 每个案件必须有法条引用
  const casesWithoutLegalRefs = result.details.cases.filter(c => !c.hasLegalReferences);
  if (casesWithoutLegalRefs.length === 0) {
    checks.passedChecks.push('✅ 所有案件都有法条引用');
  } else {
    checks.failedChecks.push(`❌ ${casesWithoutLegalRefs.length}个案件缺少法条引用`);
  }

  // 检查7: 论点数量验证
  if (result.summary.totalArguments >= 90) { // 15案件 * 2轮 * 3论点
    checks.passedChecks.push('✅ 论点数量充足');
  } else {
    checks.failedChecks.push(`❌ 论点数量不足，期望≥90个，实际${result.summary.totalArguments}个`);
  }

  // 检查8: 法条数量验证
  if (result.summary.totalLegalReferences >= 75) { // 15案件 * 5法条
    checks.passedChecks.push('✅ 法条引用数量充足');
  } else {
    checks.failedChecks.push(`❌ 法条引用数量不足，期望≥75个，实际${result.summary.totalLegalReferences}个`);
  }
  ...
  if (result.summary.estimatedCost > 0) {
    checks.passedChecks.push(`✅ AI成本估算: ¥${result.summary.estimatedCost.toFixed(4)}`);
  } else {
  ...
  if (result.summary.cacheHitRate > 0.1) {
    checks.passedChecks.push(`✅ 缓存命中率: ${(result.summary.cacheHitRate * 100).toFixed(2)}%`);
  } else {
    checks.warnings.push(`缓存命中率较低: ${(result.summary.cacheHitRate * 100).toFixed(2)}%`);
  }
...
function printValidationResults(result: ValidationResult) {
  ...
  // 元数据
  console.log('\n📅 验证时间:', new Date(result.metadata.timestamp).toLocaleString('zh-CN'));
  ...
  console.log(`   🎯 缓存命中率: ${(result.summary.cacheHitRate * 100).toFixed(2)}%`);

  // 验证结果
  console.log('\n🔍 验证结果:');
  console.log(`   总体状态: ${result.summary.validationStatus === 'PASSED' ? '✅ 通过' : '❌ 失败'}`);
  ...
  if (result.validation.passedChecks.length > 0) {
    console.log('\n✅ 通过的检查:');
    result.validation.passedChecks.forEach(check => console.log(`   ${check}`));
  }

  if (result.validation.failedChecks.length > 0) {
    console.log('\n❌ 失败的检查:');
    result.validation.failedChecks.forEach(check => console.log(`   ${check}`));
  }
  ...
  if (result.validation.criticalIssues.length > 0) {
    console.log('\n🚨 严重问题:');
    result.validation.criticalIssues.forEach(issue => console.log(`   ${issue}`));
  }
  ...
  result.details.cases.slice(0, 5).forEach(caseData => {
    ...
    console.log(`      文档: ${caseData.documentCount} | 辩论: ${caseData.debateCount} | 论点: ${caseData.argumentCount} | 法条: ${caseData.legalReferenceCount}`);
  });
