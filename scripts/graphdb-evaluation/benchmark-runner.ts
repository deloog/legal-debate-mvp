 ...
 * 基准查询类型
 */
export enum BenchmarkQueryType {
...
export interface BenchmarkConfig {
  warmupRuns?: number; // 预热运行次数（默认：3）
  benchmarkRuns?: number; // 基准运行次数（默认：10）
  recordDetailedRuns?: boolean; // 是否记录每次运行的详细信息（默认：false）
  recordPercentiles?: boolean; // 是否计算百分位数（默认：true）
  timeoutMs?: number; // 单次查询超时时间（毫秒）
...
export class BenchmarkRunner {
  private connection: GraphDatabaseConnection | null = null;
   ...
   * 设置数据库连接
   */
  setConnection(connection: GraphDatabaseConnection): void {
