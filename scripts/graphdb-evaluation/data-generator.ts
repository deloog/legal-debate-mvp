...
export class DataGenerator {
   ...
   * 生成法条
   */
  static generateArticles(count: number): TestArticle[] {
   ...
   * 生成单个法条
   */
  private static generateSingleArticle(): TestArticle {
   ...
   * 生成法律名称
   */
  static generateLegalNames(count: number): string[] {
   ...
   * 生成单个法律名称
   */
  private static generateLegalName(): string {
    const prefix = this.randomChoice(LAW_NAME_PREFIXES);
    const template = this.randomChoice(LAW_NAME_TEMPLATES);
   ...
   * 生成条文号
   */
  private static generateArticleNumber(): string {
   ...
   * 生成法条全文
   */
  static generateFullText(): string {
   ...
   * 生成额外的法律文本
   */
  private static generateExtraWords(length: number): string {
    const words = [];
    let currentLength = 0;
   ...
   * 生成生效日期
   */
  private static generateEffectiveDate(): Date {
   ...
   * 生成单个关系
   */
  private static generateSingleRelation(articleIds: string[]): TestRelation {
   ...
   * 生成测试数据集
   */
  static generateTestDataset(scale: DataScale): {
    articles: TestArticle[];
