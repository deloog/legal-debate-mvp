...
exports.EvaluationReportGenerator = void 0;
/**
 * 评估报告生成器类
 */
class EvaluationReportGenerator {
    ...
    calculateMetrics(results) {
        ...
        // 综合评分
        const overallScore = this.calculateOverallScore(avgQueryTime, successRate, stabilityScore, scalabilityScore);
        return {
            avgQueryTime,
            minQueryTime,
            maxQueryTime,
     ...
     * 生成建议
     */
    generateRecommendations(metrics, performances) {
        const recommendations = [];
        // 基于性能指标的建议
        if (metrics.avgQueryTime > 100) {
            recommendations.push(`平均查询时间较长（${metrics.avgQueryTime.toFixed(2)}ms），建议优化查询语句或添加索引`);
        }
        if (metrics.queryTimeStdDev > metrics.avgQueryTime * 0.5) {
            recommendations.push('查询时间波动较大，建议检查系统资源使用情况或优化查询缓存策略');
        }
        if (metrics.successRate < 95) {
            recommendations.push(`查询成功率较低（${metrics.successRate.toFixed(1)}%），建议检查查询语句和数据库配置`);
        }
        if (metrics.stabilityScore < 70) {
            recommendations.push(`系统稳定性评分较低（${metrics.stabilityScore.toFixed(0)}/100），建议增加系统资源或优化数据库配置`);
        }
        // 基于特定查询的建议
        const slowQueries = performances.filter(p => p.avgTime > 100);
        if (slowQueries.length > 0) {
            recommendations.push(`以下查询性能较差，建议优先优化：${slowQueries.map(q => q.queryName).join(', ')}`);
        }
        const unstableQueries = performances.filter(p => p.stdDev > p.avgTime * 0.5);
        if (unstableQueries.length > 0) {
            recommendations.push(`以下查询时间波动较大，建议检查：${unstableQueries.map(q => q.queryName).join(', ')}`);
        }
        // 性能最佳实践建议
        if (recommendations.length === 0) {
            recommendations.push('性能表现良好，建议继续监控并在数据量增长时重新评估');
        }
     ...
     * 计算稳定性评分
     */
    calculateStabilityScore(stdDev, mean) {
        if (mean === 0)
            return 100;
     ...
     * 计算综合评分
     */
    calculateOverallScore(avgTime, successRate, stabilityScore, scalabilityScore) {
        // 时间性能权重：30%
...
exports.EvaluationReportGenerator = EvaluationReportGenerator;
