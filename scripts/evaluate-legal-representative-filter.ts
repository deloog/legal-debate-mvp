...
import type { DocumentAnalysisInput, Party } from '../src/lib/agent/doc-analyzer/core/types';
 ...
 * 评估单个测试用例
 */
async function evaluateTestCase(testCase: TestCase, agent: DocAnalyzerAgent): Promise<TestResult> {
  ...
  try {
    ...
    // 调试输出
    const dataStr = result.data ? JSON.stringify(result.data).substring(0, 800) : 'undefined';
    console.log('Debug result structure:', {
      success: result.success,
      hasData: !!result.data,
      dataType: typeof result.data,
      extractedData: result.data ? (result.data as any).extractedData : undefined,
      parties: result.data ? (result.data as any).extractedData?.parties : undefined,
      dataStr
    });
    ...
    // 检查正确保留的当事人
    const correctKept = testCase.expectedParties.shouldKeep.filter(
    ...
    // 检查错误保留的当事人（应该过滤但被保留）
    const incorrectKept = testCase.expectedParties.shouldFilter.filter(
      name => actualPartyNames.includes(name)
    );
...
async function runEvaluation(): Promise<EvaluationSummary> {
  ...
  const totalParties = results.reduce(
    (sum, r) => sum + r.expectedParties.shouldKeep.length + r.expectedParties.shouldFilter.length,
    0
  );
  const correctKept = results.reduce((sum, r) => sum + r.correctKept.length, 0);
  const correctFiltered = results.reduce((sum, r) => sum + r.correctFiltered.length, 0);
  const incorrectFiltered = results.reduce((sum, r) => sum + r.incorrectFiltered.length, 0);
  const incorrectKept = results.reduce((sum, r) => sum + r.incorrectKept.length, 0);
...
${summary.results.filter(r => r.incorrectFiltered.length > 0).map(r => `- 案例${r.caseId}: ${r.incorrectFiltered.join(', ')}`).join('\n')}
...
${summary.results.filter(r => r.incorrectKept.length > 0).map(r => `- 案例${r.caseId}: ${r.incorrectKept.join(', ')}`).join('\n')}
...
async function main() {
  try {
    ...
    // 保存报告
    const reportPath = path.join(__dirname, '../docs/LEGAL_REPRESENTATIVE_FILTER_EVALUATION.md');
    ...
    console.log(`达标状态: ${summary.overallAccuracy >= 90 ? '✅ 达标' : '❌ 未达标'}`);
    console.log('========================================');
