...
class AIGeneralizationTester {
  ...
  async runGeneralizationTest(): Promise<void> {
    ...
    const testDocPath = join(__dirname, '../test-data/legal-documents/test-variation-civil-case.txt');

    console.log('📄 测试文档: variation-civil-case-001');
    console.log('📋 测试描述: 变体测试：不同人名、职业、金额、案情的民事货款纠纷');
  ...
  private printVariationResults(docId: string, result: any) {
    ...
    if (result.success) {
      ...
      result.extracted.parties.forEach((party: any, index: number) => {
        console.log(`    ${index + 1}. ${party.type}: ${party.name} (${party.role}) - ${party.contact}`);
      });
      
      console.log('\n  提取的诉讼请求:');
      result.extracted.claims.forEach((claim: any, index: number) => {
        const amount = claim.amount ? ` ¥${claim.amount.toLocaleString()}` : '';
        console.log(`    ${index + 1}. ${claim.type}: ${claim.content.substring(0, 50)}...${amount}`);
      });
    }
  }
  ...
  private generateGeneralizationReport(results: any[]) {
    ...
    console.log('\n🔬 验证结论:');
    if (successfulTests.length === results.length && successfulTests.length > 0) {
