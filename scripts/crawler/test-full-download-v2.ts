...
async function getDownloadUrl(bbbs: string): Promise<string | null> {
  ...
  try {
    ...
    if (json.code !== 200 || !json.data?.url) {
      console.log(`    ⚠ API 失败: ${json.msg}`);
      return null;
    }

    return json.data.url;
  } catch (err: any) {
    console.log(`    ⚠ 获取下载URL失败: ${err.message}`);
    return null;
  }
}
...
async function main() {
  ...
  for (const testCase of testCases) {
    ...
    // 1. 获取下载 URL
    const downloadUrl = await getDownloadUrl(testCase.bbbs);

    if (!downloadUrl) {
      console.log(`    ✗ 获取下载URL失败\n`);
      continue;
    }
