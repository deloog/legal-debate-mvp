...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5432/legal_debate_dev'
});
...
async function seedDatabase() {
  ...
  try {
    ...
    const analysisCount = await client.query('SELECT COUNT(*) as count FROM analyses');
    const msgCount = await client.query('SELECT COUNT(*) as count FROM chat_messages');
    const aiCount = await client.query('SELECT COUNT(*) as count FROM ai_interactions');
