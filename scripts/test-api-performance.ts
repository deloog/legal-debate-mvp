...
async function runPerformanceTests(): Promise<PerformanceReport> {
  ...
  for (const route of TEST_ROUTES) {
    ...
    console.log(`  ${status} ${route} - ${result.responseTime.toFixed(2)}ms (${result.statusCode})`);
  }
  ...
  console.log(`  ${streamStatus} /api/v1/debates/[id]/stream - ${streamResult.responseTime.toFixed(2)}ms (${streamResult.statusCode})`);
  ...
  for (const route of TEST_ROUTES) {
    ...
    const avgTime = concurrentResults.reduce((sum, r) => sum + r.responseTime, 0) / concurrentResults.length;
    const successCount = concurrentResults.filter(r => r.success).length;
    const status = successCount === concurrentResults.length ? '✅' : '⚠️';
    
    console.log(`  ${status} ${route} (x3) - 平均 ${avgTime.toFixed(2)}ms (${successCount}/3 成功)`);
  }
  ...
  if (report.summary.averageResponseTime < 100 && report.summary.failedTests === 0) {
    console.log('  ✅ 性能表现良好');
  }
