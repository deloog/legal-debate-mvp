...
class ImprovedDocumentAccuracyTester {
  ...
  private async testDocumentWithAIVerification(testDoc: TestDocument): Promise<ImprovedAccuracyMetrics> {
  ...
  private printImprovedResults(docId: string, description: string, metrics: ImprovedAccuracyMetrics): void {
    ...
    if (metrics.partiesAccuracy.duplicates.length > 0) {
      ...
      metrics.partiesAccuracy.duplicates.forEach(dup => {
        console.log(`    ${dup.name} (${dup.occurrences}次): ${dup.roles.join(', ')}`);
      });
    }
  ...
  private generateImprovedReport(results: ImprovedAccuracyMetrics[]): void {
    ...
    // 保存报告
    const reportPath = join(__dirname, '../test-data/improved-accuracy-report.json');
