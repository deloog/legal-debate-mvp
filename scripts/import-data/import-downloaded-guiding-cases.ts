...
// ── 内联认识论重计算 ──────────────────────────────────────────────────
async function recomputeEpistemicForArticles(
  articleIds: string[]
): Promise<void> {
  ...
  const REBUTTAL_PATTERNS = [
    {
      weight: 0.85,
      patterns: [/不(适用|符合|满足|成立|构成)/, /已(废止|失效|撤销)/, /驳回/],
    },
  ...
  function determineProfile(
    ...
    if (consensus > 0.85 && rebuttal < 0.15)
      return { profile: 'IRON_CLAD' as const, expressionGuide: '可以确定地说' };
    if (consensus > 0.65 && rebuttal > 0.2)
      return {
        profile: 'MAINSTREAM_TROUBLED' as const,
        expressionGuide: '目前仍倾向于认为',
      };
    if (inPool && consensus > 0.35 && consensus <= 0.65)
      return {
        profile: 'CANDIDATE_POOL' as const,
        expressionGuide: '目前无法确定',
      };
    if (rebuttal > 0.7 || consensus < 0.35)
      return {
        profile: 'FADING' as const,
        expressionGuide: '现在倾向于认为这一解释存在争议',
      };
    return {
      profile: 'UNCERTAIN' as const,
      expressionGuide:
  ...
  for (const articleId of articleIds) {
    const curArt = await prisma.lawArticle.findUnique({
      where: { id: articleId },
    ...
    const allVersionIds = curArt
      ? await prisma.lawArticle
          .findMany({
            where: {
              lawName: curArt.lawName,
              articleNumber: curArt.articleNumber,
            },
            select: { id: true },
          })
          .then(r => r.map(x => x.id))
    ...
    const liveSrc = refs.map(r => ({
      level: detectLevel(r.case?.court),
      jurisdiction: extractJuri(r.case?.court),
    }));
    const histSrc = histRefs.map(r => ({
      level: r.courtLevel,
    ...
    for (let i = 0; i < refs.length; i++) {
      ...
      for (const rule of REBUTTAL_PATTERNS) {
        if (
          rule.patterns.some(p => p.test(ref.content)) &&
          rule.weight > maxStr
        ) {
          maxStr = rule.weight;
          isReb = true;
      ...
      if (isReb) {
        rebuttal = Math.min(1.0, rebuttal + alpha * maxStr);
        consensus = Math.max(0.0, consensus - alpha * 0.7 * maxStr);
      }
      if (isComp) altStrength = Math.min(1.0, altStrength + alpha * 0.8);
      inertia = maxStr * w + DECAY * inertia;
      const prev = inPool;
      if (inertia > POOL_ENTRY && !inPool) {
        inPool = true;
        poolEntryDate = ref.case?.createdAt;
      } else if (inertia < POOL_EXIT && inPool) {
        inPool = false;
    ...
    for (const hist of histRefs) {
      ...
      consensus = Math.min(
        0.98,
        consensus + ALPHA * lw * 0.5 * hist.applicabilitySignal * sat * 0.1
      );
    }

    const { profile, expressionGuide } = determineProfile(
      consensus,
      rebuttal,
      inertia,
      inPool
    ...
    const totalCases =
      refs.length + histRefs.reduce((s, r) => s + r.caseCount, 0);
    const data = {
      profile,
      ...
      effectiveSourceCount: indep.effectiveCount,
      totalCasesApplied: totalCases,
      jurisdictionCount: indep.jurisdictionCount,
      courtLevelSpread: indep.courtLevelSpread,
...
async function main() {
  const dataPath =
    'C:/Users/deloo/Downloads/guiding-cases/guiding-cases/all_guiding_cases_full.json';
  ...
  for (const c of cases) {
    ...
    const matchInfo =
      unmatchedRefs.length > 0 ? ` [未匹配: ${unmatchedRefs.join(', ')}]` : '';
    log(
      `  ✓ ${c.case_no} 第${c.batch}批 ${year}年「${c.title.slice(0, 22)}」→ ${matchedIds.length} 条法条${matchInfo}`
    );
  }
