...
function normalizeArticleNumber(raw: string): string {
  ...
  if (arabicMatch)
    return `第${arabicToChinese(parseInt(arabicMatch[1], 10))}条`;
  // 已是中文格式
...
async function buildIndex(): Promise<Map<string, string>> {
  ...
  log(
    `法条索引：${map.size} 条（${new Set(articles.map(a => a.lawName)).size} 部法律）`
  );
  return map;
}
...
function lookupId(
  ...
  for (const suffix of [6, 8, 10]) {
    ...
    for (const [key, id] of index) {
      if (key.endsWith(`::${articleNumber}`) && key.includes(tail))
        return { id, isMapped: false };
    }
  }
  // 4. 废止法律 → 民法典 回退映射
...
async function recompute(articleIds: string[]): Promise<void> {
  ...
  const REBUTTAL_RULES = [
    {
      w: 0.85,
      re: [/不(适用|符合|满足|成立|构成)/, /已(废止|失效|撤销)/, /驳回/],
    },
  ...
  const indep = (srcs: { level: string; jurisdiction: string }[]) => {
    ...
    for (const s of srcs) {
      const k = `${s.jurisdiction}::${s.level}`;
      cls.set(k, (cls.get(k) ?? 0) + 1);
    }
    let eff = 0;
    for (const [, sz] of cls) eff += 1 / Math.sqrt(Math.max(1, sz));
    return {
      independence: Math.min(1, eff / srcs.length),
      effectiveCount: Math.round(eff * 100) / 100,
      jurisdictionCount: new Set(
        srcs.map(s => s.jurisdiction).filter(j => j !== 'UNKNOWN')
      ).size,
      courtLevelSpread: new Set(
        srcs.map(s => s.level).filter(l => l !== 'UNKNOWN')
      ).size,
    };
  };
  const profile = (
    ...
    if (consensus > 0.85 && rebuttal < 0.15) {
      prof = 'IRON_CLAD';
      guide = '可以确定地说';
    } else if (consensus > 0.65 && rebuttal > 0.2) {
      prof = 'MAINSTREAM_TROUBLED';
      guide = '目前仍倾向于认为';
    } else if (inPool && consensus > 0.35 && consensus <= 0.65) {
      prof = 'CANDIDATE_POOL';
      guide = '目前无法确定';
    } else if (rebuttal > 0.7 || consensus < 0.35) {
      prof = 'FADING';
      guide = '现在倾向于认为这一解释存在争议';
    } else {
      prof = 'UNCERTAIN';
      guide =
    ...
    if (hasAmended)
      guide += `（注：该条文存在修正案历史版本，引用历史案例时须核查当时适用版本与现行条文的差异）`;
  ...
  for (const articleId of articleIds) {
    const cur = await prisma.lawArticle.findUnique({
      where: { id: articleId },
    ...
    const allIds = cur
      ? await prisma.lawArticle
          .findMany({
            where: { lawName: cur.lawName, articleNumber: cur.articleNumber },
            select: { id: true },
          })
          .then(r => r.map(x => x.id))
      : [articleId];
    const hasAmended =
      allIds.length > 1 &&
      (await prisma.lawArticle
        .count({ where: { id: { in: allIds }, status: 'AMENDED' } })
        .then(c => c > 0));
    ...
    const [refs, hists] = await Promise.all([
      prisma.legalReference.findMany({
        ...
        include: {
          case: { select: { id: true, court: true, createdAt: true } },
        },
        orderBy: { createdAt: 'asc' },
      }),
      prisma.historicalCaseArticleRef.findMany({
        where: { lawArticleId: { in: allIds } },
      }),
    ]);

    const ls = refs.map(r => ({
      level: detectLv(r.case?.court),
      jurisdiction: detectJu(r.case?.court),
    }));
    const hs = hists.map(r => ({
      level: r.courtLevel,
    ...
    for (let i = 0; i < refs.length; i++) {
      ...
      for (const rule of REBUTTAL_RULES) {
        if (rule.re.some(p => p.test(ref.content)) && rule.w > maxStr) {
          maxStr = rule.w;
          isR = true;
        }
      }
      ...
      if (isR) {
        rb = Math.min(1, rb + alpha * maxStr);
        cs = Math.max(0, cs - alpha * 0.7 * maxStr);
      }
      ...
      if (in_ > PE && !ip) {
        ip = true;
        ped = ref.case?.createdAt;
      } else if (in_ < PX && ip) {
        ip = false;
        pxd = ref.case?.createdAt;
      }
    ...
    for (const h of hists) {
      ...
      cs = Math.min(
        0.98,
        cs + ALPHA * lw * 0.5 * h.applicabilitySignal * sat * 0.1
      );
    }
    const { profile: prof, expressionGuide: eg } = profile(
      cs,
      rb,
      in_,
      ip,
    ...
    const d = {
      profile: prof,
      consensusScore: Math.round(cs * 1000) / 1000,
      challengePressure: Math.round(rb * 1000) / 1000,
      inertia: Math.round(in_ * 1000) / 1000,
      inCandidatePool: ip,
      alternativeStrength: Math.round(alt * 1000) / 1000,
      sourceIndependence: Math.round(ind.independence * 1000) / 1000,
      effectiveSourceCount: ind.effectiveCount,
      totalCasesApplied: tot,
      jurisdictionCount: ind.jurisdictionCount,
      courtLevelSpread: ind.courtLevelSpread,
      minorityPressureCount: min,
      poolEntryDate: ped,
      poolExitDate: pxd,
      expressionGuide: eg,
      lastCaseId: lid ?? null,
    ...
    await prisma.lawArticleEpistemicState.upsert({
      ...
      create: { lawArticleId: articleId, ...d },
...
async function main() {
  const dataPath =
    'C:/Users/deloo/Downloads/guiding-cases (1)/guiding-cases/guiding_cases_final_v3.json';
  ...
  const overrides: Record<string, { reason: string; articles: string[] }> =
    fs.existsSync(overridesPath)
      ? (
          JSON.parse(fs.readFileSync(overridesPath, 'utf-8')) as {
            overrides: Record<string, { reason: string; articles: string[] }>;
          }
        ).overrides
      : {};
  ...
  // 先清除旧数据（保证幂等）
  const cleared = await prisma.historicalCaseArticleRef.deleteMany({
  ...
  for (const c of cases) {
    ...
    for (const ref of refs) {
      ...
      if (result) {
        ...
        if (result.isMapped) {
          mappedNotes.push(
            `${ref.lawName.replace(/^中华人民共和国/, '')}${ref.articleNumber}→民法典`
          );
        }
      } else {
    ...
    if (matchedIds.length === 0) {
      log(
        `  ⚠ ${caseNo}「${c.title?.slice(0, 20)}」— DB无匹配 [${unmatched.join('|')}]`
      );
      noMatch++;
      continue;
    }
    ...
    const unmatchedNote =
      unmatched.length > 0 ? ` [未匹配:${unmatched.join(',')}]` : '';
    const mappedNote =
      mappedNotes.length > 0 ? ` [废止映射:${mappedNotes.join(',')}]` : '';
    log(
      `  ✓ ${caseNo} 第${c.batch}批「${c.title?.slice(0, 22)}」→ ${matchedIds.length}条${mappedNote}${unmatchedNote}`
    );
  }
  ...
  const total = await prisma.historicalCaseArticleRef.count({
    where: { dataSource: 'SUPREME_GUIDING' },
  });
  ...
  const allIds = [
    ...new Set([...affectedIds, ...allAffected.map(r => r.lawArticleId)]),
  ];
  await recompute(allIds);

  const dist = await prisma.lawArticleEpistemicState.groupBy({
    by: ['profile'],
    _count: { id: true },
  });
  log('\n最终认识论状态分布：');
