...
async function buildArticleIndex(): Promise<Map<string, string>> {
  ...
  log(
    `  法条索引已加载：${map.size} 条（${new Set(articles.map(a => a.lawName)).size} 部法律）`
  );
  return map;
}
...
async function importCases(
  ...
  for (const c of cases) {
      ...
      .map(a => {
        ...
        if (!id) {
          ...
          for (const [k, v] of articleIndex) {
            if (
              k.includes(a.articleNumber) &&
              k.includes(a.lawName.slice(-4))
            ) {
              return {
                key,
                id: v,
    ...
    if (matchedArticles.length === 0) {
      log(
        `  ⚠ ${c.caseNumber}「${c.name.slice(0, 20)}」— 无法匹配任何法条，跳过`
      );
      noMatch++;
      continue;
    }
    ...
    log(
      `  ✓ ${c.caseNumber}「${c.name.slice(0, 25)}」→ ${matchedArticles.length} 条法条`
    );
  }
...
async function main() {
  ...
  log(
    `合并后（去重）：${cases.length} 个案例，共引用 ${cases.reduce((s, c) => s + c.articles.length, 0)} 条法条`
  );
  ...
  // 统计写入结果
  const count = await prisma.historicalCaseArticleRef.count({
...
async function recomputeEpistemicStates(articleIds: string[]): Promise<void> {
  ...
  const REBUTTAL_PATTERNS = [
    ...
    {
      weight: 0.5,
      patterns: [/仅(适用|限于|针对)/, /除非|前提是/, /限于|仅在|只有当/],
    },
    {
      weight: 0.65,
      patterns: [/应(适用|优先适用)/, /特别法|优先(于|适用)/, /竞合|冲突/],
    },
    {
      weight: 0.25,
      patterns: [/难以(认定|支持)/, /值得(商榷|质疑)/, /尚(存争议|无定论)/],
    },
  ];
      ...
      prisma.lawArticle
        ...
        .then(a =>
          a
            ? prisma.lawArticle
                .findMany({
                  where: { lawName: a.lawName, articleNumber: a.articleNumber },
                  select: { id: true },
                })
                .then(rows =>
                  prisma.historicalCaseArticleRef.findMany({
                    where: { lawArticleId: { in: rows.map(r => r.id) } },
                  })
                )
            : prisma.historicalCaseArticleRef.findMany({
