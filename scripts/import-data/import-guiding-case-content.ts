...
function arabicToChinese(n: number): string {
  ...
  const t = Math.floor(r2 / 10),
    o = r2 % 10;
  return (
    base + mid + (t > 0 ? CN[t] + '十' + (o > 0 ? CN[o] : '') : '零' + CN[o])
  );
}
...
function buildRepealedIndex(index: Map<string, string>): Map<string, string> {
  ...
  const file = JSON.parse(fs.readFileSync(mappingPath, 'utf-8')) as {
    mappings: Array<{
      ...
      aliases: string[];
      toLaw: string;
      articles: Array<{ from: string; to: string | null }>;
    }>;
  };
  ...
  for (const group of file.mappings) {
    for (const art of group.articles) {
      ...
      for (const fromName of [group.fromLaw, ...group.aliases]) {
        ...
        repMap.set(
          `${fromName.replace(/^中华人民共和国/, '')}::${art.from}`,
          toId
        );
      }
    }
...
function lookupId(
  ...
  return (
    index.get(`${lawName}::${articleNumber}`) ??
...
async function main() {
  ...
  const overrides: Record<string, { articles: string[] }> = fs.existsSync(
    overridesPath
  )
    ? (
        JSON.parse(fs.readFileSync(overridesPath, 'utf-8')) as {
          overrides: Record<string, { articles: string[] }>;
        }
      ).overrides
    : {};
  ...
  for (const c of rawData.cases) {
    const caseNo =
      typeof c.case_no === 'string' ? parseInt(c.case_no, 10) : c.case_no;
    ...
    const relevantLawsText = Array.isArray(c.relevant_laws)
      ? c.relevant_laws
          .filter(s => !s.includes('版权所有') && !s.includes('法律声明'))
          .join('；')
  ...
  const linked = await prisma.guidingCase.count({
    where: { lawArticles: { some: {} } },
  });
