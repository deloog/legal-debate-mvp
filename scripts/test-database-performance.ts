import { prisma, checkDatabaseConnection, getConnectionInfo } from '../src/lib/db/prisma';
...
class DatabasePerformanceTester {
  ...
  async testQueryPerformance(): Promise<void> {
    ...
    console.log(`  成功率: ${((this.metrics.successCount / queries.length) * 100).toFixed(1)}%`);
  }
  ...
  generatePerformanceReport(): void {
    ...
    console.log(`  性能评级: ${avgQueryTime < 100 ? '✅ 优秀' : avgQueryTime < 500 ? '⚠️ 良好' : '❌ 需要优化'}`);
    
    // 并发能力评估
    console.log('\n🔄 并发处理能力:');
    console.log(`  最大并发测试: ${this.metrics.concurrentQueries}`);
    console.log(`  并发处理: ${this.metrics.concurrentQueries >= 10 ? '✅ 满足要求' : '⚠️ 需要提升'}`);
    
    // 连接池状态
    console.log('\n🏊 连接池状态:');
    if (this.metrics.poolStats) {
      console.log(`  连接池配置: ✅ 正常`);
      console.log(`  连接管理: ${this.metrics.poolStats.totalConnections ? '✅ 正常' : '⚠️ 需要检查'}`);
    } else {
    ...
    console.log(`  查询性能: ${queryPerformanceGood ? '✅ 通过' : '❌ 不达标'}`);
    console.log(`  并发能力: ${concurrencyGood ? '✅ 通过' : '❌ 不达标'}`);
    console.log(`  错误率: ${errorRateLow ? '✅ 通过' : '❌ 过高'}`);
    console.log(`  总体评估: ${allGood ? '✅ 性能满足要求' : '❌ 性能需要优化'}`);
