...
class DocumentAccuracyTester {
  ...
  private async testDocumentAccuracy(testDoc: TestDocument): Promise<AccuracyMetrics> {
    try {
      // 初始化Agent
      ...
      // 计算总体准确率
      const overallAccuracy = (partiesMetrics.accuracy + claimsMetrics.recall) / 2;
  ...
  private calculatePartiesAccuracy(
    ...
    for (const expectedParty of expected) {
      ...
      if (found) {
        ...
        if (legalRoles.includes(found.role) && expectedRoles.includes(expectedParty.role)) {
  ...
  private calculateClaimsRecall(
    ...
    for (const expectedClaim of expected) {
      const found = extracted.find(extracted => {
        ...
        const amountMatch = expectedClaim.amount === undefined || 
  ...
  private printResults(docId: string, metrics: AccuracyMetrics): void {
    ...
    console.log(`  当事人信息准确率: ${(metrics.partiesAccuracy * 100).toFixed(1)}%`);
    console.log(`  诉讼请求召回率: ${(metrics.claimsRecall * 100).toFixed(1)}%`);
    console.log(`  总体准确率: ${(metrics.overallAccuracy * 100).toFixed(1)}%`);
    console.log(`  详细统计:`);
    console.log(`    当事人: ${metrics.detailedResults.partiesCorrect}/${metrics.detailedResults.partiesExpected} 正确`);
    console.log(`    诉讼请求: ${metrics.detailedResults.claimsCorrect}/${metrics.detailedResults.claimsExpected} 正确`);
  }

  private generateReport(results: AccuracyMetrics[]): void {
    const avgPartiesAccuracy = results.reduce((sum, r) => sum + r.partiesAccuracy, 0) / results.length;
    const avgClaimsRecall = results.reduce((sum, r) => sum + r.claimsRecall, 0) / results.length;
    ...
    console.log('\n🎯 总体测试结果:');
    console.log(`  平均当事人信息准确率: ${(avgPartiesAccuracy * 100).toFixed(1)}% (目标: ≥98%)`);
    console.log(`  平均诉讼请求召回率: ${(avgClaimsRecall * 100).toFixed(1)}% (目标: ≥95%)`);
    console.log(`  平均总体准确率: ${(avgOverallAccuracy * 100).toFixed(1)}%`);
    console.log(`  验收标准: ${report.summary.overallPassed ? '✅ 通过' : '❌ 未通过'}`);
    console.log(`\n📄 详细报告已保存至: ${reportPath}`);
  }
}
