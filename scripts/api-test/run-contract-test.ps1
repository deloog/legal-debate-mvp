...
$TEST_EMAIL = if ($Email) { $Email } else { "lawyer@example.com" }
$TEST_PASSWORD = if ($Password) { $Password } else { "lawyer123" }
...
# 帮助信息
if ($Help) {
    Write-Host "${Blue}合同管理 API 测试脚本 (PowerShell)${NC}"
    Write-Host ""
    Write-Host "用法: .\run-contract-test.ps1 [选项]"
    Write-Host ""
    Write-Host "选项:"
    Write-Host "  -Help                    显示帮助信息"
    Write-Host "  -Url URL                 设置 API 基础 URL (默认: http://localhost:3000)"
    Write-Host "  -Email EMAIL             设置测试用户邮箱 (默认: lawyer@example.com)"
    ...
    Write-Host ""
    Write-Host "注意: 合同创建需要 LAWYER 或 SUPER_ADMIN 角色，且律师用户需要有通过的资质审核"
    Write-Host ""
    Write-Host "示例:"
    Write-Host "  .\run-contract-test.ps1                                    # 使用默认配置运行"
    Write-Host "  .\run-contract-test.ps1 -Url http://localhost:3001         # 指定不同的端口"
    Write-Host "  .\run-contract-test.ps1 -Email admin@example.com -Password admin123"
    exit 0
}
...
Write-Host ""
Write-Host "${Cyan}╔══════════════════════════════════════════════════════════════╗${NC}"
Write-Host "${Cyan}║          📋 合同管理业务场景 API 测试                        ║${NC}"
Write-Host "${Cyan}╚══════════════════════════════════════════════════════════════╝${NC}"
Write-Host ""
...
Write-Host "  用户角色要求: LAWYER 或 SUPER_ADMIN"
Write-Host ""
Write-Host "${Yellow}ℹ️  注意: 合同创建需要律师资质审核通过${NC}"
Write-Host ""
...
# 运行测试
Write-Host ""
Write-Host "${Blue}🚀 开始运行合同管理 API 测试...${NC}"
Write-Host "================================================================"
Write-Host ""
...
try {
    npx ts-node --project tsconfig.json scripts/api-test/contract-workflow-test.ts
...
if ($TEST_EXIT_CODE -eq 0) {
    Write-Host "${Green}✅ 所有合同管理测试通过!${NC}"
} else {
    Write-Host "${Red}❌ 测试失败，请查看上方错误信息${NC}"
    Write-Host ""
    Write-Host "${Yellow}常见问题:${NC}"
    Write-Host "  1. 403 Forbidden - 用户角色不是 LAWYER/SUPER_ADMIN"
    Write-Host "  2. 403 QUALIFICATION_REQUIRED - 律师资质未通过审核"
    Write-Host "  3. 401 Unauthorized - 邮箱或密码错误"
    Write-Host "  4. 500 Internal Error - 服务器内部错误，请检查日志"
}
