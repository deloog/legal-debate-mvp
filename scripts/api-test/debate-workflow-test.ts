...
class ApiClient {
  ...
  async getCases(
    page = 1,
    limit = 10
  ): Promise<ApiResponse<{ cases: Case[]; total: number }>> {
  ...
  // 辩论状态管理
  async getDebateStatus(id: string): Promise<
    ApiResponse<{
      id: string;
      status: string;
...
async function main() {
  ...
  // ==========================================================================
  const testEmail =
    CONFIG.TEST_USER.email || `api-test-${Date.now()}@example.com`;
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          ...
          console.log(
            `   ⚠️  尝试第 ${attempt + 1}/${maxRetries} 次注册: ${currentEmail}`
          );
        }
  ...
  runner.test('2.1 创建测试案件', async () => {
    ...
    assert(
      response.data?.title?.includes('API测试案件') === true,
      '案件标题应该匹配'
    );
  ...
  runner.test('4.1 获取辩论状态', async () => {
    ...
    assertEquals(response.data?.status, 'DRAFT', 'status should be DRAFT');
    assert(
      Array.isArray(response.data?.availableTransitions),
      'availableTransitions should be an array'
    );
    assert(
      response.data!.availableTransitions.includes('IN_PROGRESS'),
      'should allow transition to IN_PROGRESS'
    );
  });

  runner.test('4.2 更新辩论状态: DRAFT -> IN_PROGRESS', async () => {
    const response = await client.updateDebateStatus(
      testData.testDebate!.id,
      'IN_PROGRESS'
    );
    ...
    assertEquals(
      response.data?.previousStatus,
      'DRAFT',
      'previous status should be DRAFT'
    );
    assertEquals(
      response.data?.newStatus,
      'IN_PROGRESS',
      'new status should be IN_PROGRESS'
    );
  });
  ...
  runner.test('5.1 创建辩论轮次', async () => {
    ...
    assertEquals(
      response.data?.debateId,
      testData.testDebate!.id,
      'debateId should match'
    );
    assertEquals(
      response.data?.roundNumber,
      2,
      'roundNumber should be 2 (1 is auto-created)'
    );

    testData.testRound = response.data;
    console.log(
      `   🔄 轮次ID: ${testData.testRound?.id} (第${testData.testRound?.roundNumber}轮)`
    );
  });
  ...
  runner.test('7.2 获取法条引用', async () => {
    ...
    if (
      roundsResponse.success &&
      roundsResponse.data &&
      roundsResponse.data.length > 0
    ) {
      const firstRound = roundsResponse.data[0];
      const response = await client.getLegalReferences(
        testData.testDebate!.id,
        firstRound.id
      );
