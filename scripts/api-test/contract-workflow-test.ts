...
class ApiClient {
  ...
  > {
    ...
    if (filters?.keyword)
      query += `&keyword=${encodeURIComponent(filters.keyword)}`;
  ...
  async signContract(
    ...
    ApiResponse<{ role: string; signedAt: string; isFullySigned: boolean }>
  > {
    return this.request('POST', `/api/contracts/${id}/sign`, {
      role,
      signature,
    });
  }
  ...
  async startApproval(
    ...
    return this.request('POST', `/api/contracts/${contractId}/approval/start`, {
      templateId,
    });
  }
  ...
  async cancelApproval(contractId: string): Promise<ApiResponse<void>> {
    return this.request(
      'POST',
      `/api/contracts/${contractId}/approval/cancel`,
      {}
    );
  }
  ...
  async executeContract(
    ...
    return this.request('POST', `/api/contracts/${contractId}/execute`, {
      action,
      data,
    });
  }
  ...
  async removeContractLawArticle(
    contractId: string,
    articleId: string
  ...
  // 法条推荐
  async getLawRecommendations(
    contractId: string
  ): Promise<ApiResponse<unknown>> {
    return this.request(
      'GET',
      `/api/v1/contracts/${contractId}/law-recommendations`
    );
  }
  ...
  async rollbackContractVersion(
    contractId: string,
    versionId: string
  ): Promise<ApiResponse<Contract>> {
    return this.request(
      'POST',
      `/api/contracts/${contractId}/versions/rollback`,
      { versionId }
    );
  }
  ...
  async sendContractEmail(
    ...
    return this.request(
      'POST',
      `/api/contracts/${contractId}/send-email`,
      emailData
    );
  }
}
...
async function main() {
  ...
  // ==========================================================================
  const testEmail =
    CONFIG.TEST_USER.email || `contract-test-${Date.now()}@example.com`;
  ...
  runner.test('1.0 获取访问令牌（登录或注册）', async () => {
    ...
    if (response && response.success) {
      ...
      client.setToken(testData.token);
      console.log(
        `   ✨ 新律师用户注册成功: ${testData.user?.email} (${testData.user?.role})`
      );
    } else {
  ...
  runner.test('3.1 创建合同（草稿状态）', async () => {
    try {
      ...
      console.log(
        `   📄 LawyerID: ${response.data?.lawyerId || 'auto-filled'}`
      );
  ...
  runner.test('3.3 获取合同列表（带筛选）', async () => {
    ...
    assert(
      response.data!.items[0].clientName.includes('王五'),
      '应该找到客户王五的合同'
    );
  });
  ...
  runner.test('4.2 获取审批信息', async () => {
    ...
    assertEquals(
      response.data?.id,
      testData.testApproval!.id,
      'approval id should match'
    );
  });
  ...
  runner.test('5.1 律师签署合同', async () => {
    ...
    const response = await client.signContract(
      testData.testContract!.id,
      'lawyer',
      mockSignature
    );
    ...
    assertEquals(
      response.data?.isFullySigned,
      false,
      'isFullySigned should be false'
    );
    console.log('   ✓ 律师已签署');
  });
  ...
  runner.test('5.3 验证合同状态已变为 SIGNED', async () => {
    ...
    assertEquals(
      response.data?.status,
      'SIGNED',
      'contract status should be SIGNED'
    );
  ...
  runner.test('6.3 验证合同已付金额已更新', async () => {
    ...
    assert(response.success === true, '获取合同详情应该成功');
    assert(
      response.data!.paidAmount >= 10000,
      'paidAmount should include new payment'
    );
  ...
  runner.test('7.1 获取合同关联法条', async () => {
    if (skipIfNoContract(testData)) return;
    const response = await client.getContractLawArticles(
      testData.testContract!.id
    );
  ...
  runner.test('7.2 获取法条推荐', async () => {
    if (skipIfNoContract(testData)) return;
    const response = await client.getLawRecommendations(
      testData.testContract!.id
    );
  ...
  runner.test('8.1 获取合同版本历史', async () => {
    if (skipIfNoContract(testData)) return;
    const response = await client.getContractVersions(
      testData.testContract!.id
    );
  ...
  runner.test('10.1 开始执行合同', async () => {
    if (skipIfNoContract(testData)) return;
    try {
      const response = await client.executeContract(
        testData.testContract!.id,
        'START_EXECUTION'
      );
