...
function filterLegalRepresentatives(text: string, parties: Party[]): Party[] {
  const legalRepKeywords = ['法定代表人', '法定代表', '法人代表', '负责人', '执行事务合伙人'];
  const independentRoles = ['原告', '被告', '第三人', '申请人', '被申请人', '上诉人', '被上诉人'];
  
  return parties.filter((party: Party) => {
    // 如果角色包含独立角色关键词，直接保留
    if (party.role && independentRoles.some(role => party.role!.includes(role))) {
      return true;
    }
    
    // 如果角色包含法定代表人关键词，过滤掉
    if (party.role && legalRepKeywords.some(keyword => party.role!.includes(keyword))) {
      return false;
    }
    
    // 检查名称在上下文中是否紧跟法定代表人关键词
    const patterns = legalRepKeywords.map(keyword => 
      new RegExp(`${keyword}[:：]?\\s*${party.name}`, 'g')
    );
...
async function runTests(): Promise<void> {
  ...
  for (const testCase of testCases) {
    ...
    // 过滤法定代表人
    const filteredParties = filterLegalRepresentatives(testCase.content, extractedParties);
