...
const DECAY = 0.78;
const ALPHA = 0.2;
const POOL_ENTRY_THRESHOLD = 0.4;
...
function computeSourceIndependence(
  sources: Array<{ level: string; jurisdiction: string }>
) {
  if (sources.length === 0)
    return {
      independence: 0,
  ...
  for (const [, sz] of clusters)
    effectiveCount += 1.0 / Math.sqrt(Math.max(1, sz));
...
async function computeForArticle(articleId: string): Promise<void> {
  ...
  const histSources = historicalRefs.map(r => ({
    level: r.courtLevel,
  ...
  for (let i = 0; i < references.length; i++) {
    ...
    if (REBUTTAL_PATTERNS[2].patterns.some(p => p.test(ref.content)))
      isCompReplacement = true;
    if (maxStrength > 0.3 && consensus > 0.75) minorityCount++;
    if (isSupport && !isRebuttal)
      consensus = Math.min(0.98, consensus + alpha * 0.3);
    if (isRebuttal) {
      rebuttal = Math.min(1.0, rebuttal + alpha * maxStrength);
      consensus = Math.max(0.0, consensus - alpha * 0.7 * maxStrength);
    }
    if (isCompReplacement)
      alternativeStrength = Math.min(1.0, alternativeStrength + alpha * 0.8);
    inertia = maxStrength * w + DECAY * inertia;
    const prevPool = inPool;
    if (inertia > POOL_ENTRY_THRESHOLD && !inPool) {
      inPool = true;
      poolEntryDate = ref.case?.createdAt;
    } else if (inertia < POOL_EXIT_THRESHOLD && inPool) {
      inPool = false;
  ...
  for (const hist of historicalRefs) {
    ...
    consensus = Math.min(
      0.98,
      consensus + ALPHA * levelWeight * 0.5 * supportStrength
    );
  }
  ...
  const hasAmended =
    ...
    (await prisma.lawArticle
      .count({ where: { id: { in: allVersionIds }, status: 'AMENDED' } })
      .then(c => c > 0));
...
async function main() {
  ...
  log(
    `需计算法条数：${ids.length}（实时引用 ${liveRefs.length} 条，历史聚合 ${historicalRefs.length} 个）`
  );
  ...
  const distribution = await prisma.lawArticleEpistemicState.groupBy({
    by: ['profile'],
    _count: { id: true },
  });
  log('状态分布：');
