...
import { createSuccessResponse, createCreatedResponse } from '@/app/api/lib/responses/api-response';
import { validateRequestBody, validatePathParam } from '@/app/api/lib/validation/validator';
import { uuidSchema, createDebateRoundSchema } from '@/app/api/lib/validation/schemas';
 ...
 * 创建404响应的辅助函数
 */
function createNotFoundResponse(message: string = 'Resource not found'): NextResponse {
...
export const POST = withErrorHandler(async (request: NextRequest, context: { params: { id: string } }) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // 检查辩论状态
    if (debate.status !== DebateStatus.DRAFT && debate.status !== DebateStatus.IN_PROGRESS) {
