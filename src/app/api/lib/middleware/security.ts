 ...
 * CORS中间件
 */
export const corsMiddleware: Middleware = async (request, context, response) => {
 ...
 * 安全头中间件
 */
export const securityMiddleware: Middleware = async (request, context, response) => {
...
export const rateLimitMiddleware: Middleware = async (request, context, response) => {
  ...
  if (!rateLimiter.isAllowed(key, limit, window)) {
    ...
    response.headers.set('X-RateLimit-Reset', new Date(Date.now() + window).toISOString());
