...
describe('LawArticleSearchService', () => {
  describe('search 方法', () => {
    ...
    it('应该返回正确的检索结果结构', async () => {
      ...
      if (result.results.length > 0) {
        ...
        expect(result.results[0].matchDetails).toHaveProperty('popularityScore');
      }
    });
  ...
  describe('getArticleById 方法', () => {
    ...
    it('应该对不存在的ID返回null', async () => {
      const article = await LawArticleSearchService.getArticleById(
        'non-existent-id',
      );
  ...
  describe('getArticleByLawNameAndNumber 方法', () => {
    it('应该根据法律名称和法条编号获取法条', async () => {
      const article = await LawArticleSearchService.getArticleByLawNameAndNumber(
        '中华人民共和国民法典',
        '第五百七十七条',
      );
      ...
      if (searchResult.results.length > 0) {
        ...
        if (similarArticles.length > 0) {
          ...
          for (let i = 1; i < similarArticles.length; i++) {
            expect(similarArticles[i - 1].similarityScore).toBeGreaterThanOrEqual(
              similarArticles[i].similarityScore,
            );
          }
        }
      }
  ...
  describe('getSuggestions 方法', () => {
    it('应该提供搜索建议', async () => {
      const suggestions = await LawArticleSearchService.getSuggestions('民法典', undefined, 5);
