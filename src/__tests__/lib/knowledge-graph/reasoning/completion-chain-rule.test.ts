...
describe('CompletionChainRule', () => {
  let rule: CompletionChainRule;
  let context: ReasoningContext;

  beforeEach(() => {
    rule = new CompletionChainRule();
  ...
  describe('补全关系链推理', () => {
    it('应该正确推断间接补全关系（2跳）', async () => {
      ...
      expect(twoHopInference?.inferredRelation).toBe(RelationType.COMPLETES);
    ...
    it('应该支持多跳传递（3跳）', async () => {
      ...
      // 应该推断出article-1间接补全article-3和article-4
  ...
  describe('推理说明', () => {
    it('应该生成清晰的推理说明', async () => {
      ...
      expect(inferences[0].explanation).toContain('间接补全');
    });
  });
