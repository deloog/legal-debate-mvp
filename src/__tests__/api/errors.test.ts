...
import { ApiError, ValidationError, NotFoundError, UnauthorizedError, ForbiddenError, InternalServerError } from '@/app/api/lib/errors/api-error';
import { handleApiError, withErrorHandler, withErrorHandling } from '@/app/api/lib/errors/error-handler';
...
describe('API Error Handling', () => {
  ...
  describe('InternalServerError', () => {
    ...
    it('should handle InternalServerError with details', () => {
      const error = new InternalServerError('Database error', { query: 'SELECT * FROM users' });
  ...
  describe('handleApiError', () => {
    ...
    it('should handle Zod validation error', async () => {
      ...
      expect(responseData.error.details.validationErrors).toEqual(zodError.issues);
    });
  ...
  describe('withErrorHandler', () => {
    ...
    it('should pass through successful responses', async () => {
      const mockResponse = { status: 200, json: jest.fn().mockReturnValue({ success: true }) };
  ...
  describe('Error Response Format', () => {
    ...
    it('should maintain consistent error response structure', async () => {
      ...
      const error = new ApiError(400, 'TEST_ERROR', 'Test message', { detail: 'value' });
      const response = handleApiError(error, mockRequest);
      const responseData = await response.json();
