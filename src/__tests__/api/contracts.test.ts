...
jest.mock('@/app/api/lib/middleware/contract-auth', () => ({
  ...
  unauthorizedResponse: () => 
    new Response(JSON.stringify({ success: false, error: { code: 'UNAUTHORIZED', message: '请先登录' } }), {
      status: 401,
      headers: { 'Content-Type': 'application/json' },
    }),
  forbiddenResponse: () =>
    new Response(JSON.stringify({ success: false, error: { code: 'FORBIDDEN', message: '无权操作此合同' } }), {
      status: 403,
      headers: { 'Content-Type': 'application/json' },
    }),
}));
...
const mockPrisma = {
  ...
  $transaction: jest.fn((callbacks: any[]) => Promise.all(callbacks.map(cb => cb()))),
};
...
import { GET as GETPayments, POST as POSTPayment } from '@/app/api/contracts/[id]/payments/route';
...
describe('Contract API Tests', () => {
  ...
  describe('GET /api/contracts/[id]', () => {
    it('should get contract details', async () => {
      ...
      const request = createMockRequest(`http://localhost:3000/api/contracts/${testContractId}`);
      const response = await GETById(request, { params: Promise.resolve({ id: testContractId }) });
      const result = await response.json();
    ...
    it('should return 404 for non-existent contract', async () => {
      ...
      const request = createMockRequest('http://localhost:3000/api/contracts/non-existent-id');
  ...
  describe('PUT /api/contracts/[id]', () => {
    it('should update contract', async () => {
      ...
      const response = await PUTById(request, { params: Promise.resolve({ id: testContractId }) });
      const result = await response.json();
  ...
  describe('GET /api/contracts/[id]/payments', () => {
    it('should get payment records', async () => {
      ...
      const mockPayments = [
        { id: 'payment-1', paymentType: '首付款', amount: 5000, status: 'PAID' },
        { id: 'payment-2', paymentType: '尾款', amount: 5000, status: 'PENDING' },
      ];

      mockPrisma.contract.findUnique.mockResolvedValue(mockContract);
      mockPrisma.contractPayment.findMany.mockResolvedValue(mockPayments);

      const request = createMockRequest(`http://localhost:3000/api/contracts/${testContractId}/payments`);
      const response = await GETPayments(request, { params: Promise.resolve({ id: testContractId }) });
      const result = await response.json();
  ...
  describe('POST /api/contracts/[id]/payments', () => {
    it('should create payment record', async () => {
      ...
      mockPrisma.contract.findUnique.mockResolvedValue(mockContract);
      mockPrisma.contractPayment.aggregate.mockResolvedValue({ _sum: { amount: 0 } });
      ...
      const response = await POSTPayment(request, { params: Promise.resolve({ id: testContractId }) });
      const result = await response.json();
