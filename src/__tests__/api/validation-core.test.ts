...
describe('Validation Core', () => {
  describe('validateRequestBody', () => {
    ...
    it('should throw ValidationError for Zod validation errors', async () => {
      ...
      await expect(validateRequestBody(mockRequest, schema)).rejects.toThrow(ValidationError);
    });
    ...
    it('should throw ValidationError for invalid JSON', async () => {
      ...
      await expect(validateRequestBody(invalidRequest, schema)).rejects.toThrow(ValidationError);
      await expect(validateRequestBody(invalidRequest, schema)).rejects.toThrow('Invalid JSON in request body');
    });
    ...
    it('should throw ValidationError for malformed JSON that throws during parsing', async () => {
      ...
      malformedRequest.json = jest.fn().mockRejectedValue(new Error('Unexpected token'));

      const schema = z.object({ test: z.string() });

      await expect(validateRequestBody(malformedRequest, schema)).rejects.toThrow(ValidationError);
      await expect(validateRequestBody(malformedRequest, schema)).rejects.toThrow('Invalid JSON in request body');
    ...
    it('should handle empty request body', async () => {
      ...
      await expect(validateRequestBody(emptyRequest, schema)).rejects.toThrow(ValidationError);
    });
  ...
  describe('validateQueryParams', () => {
    ...
    it('should handle array parameters', () => {
      const url = 'http://localhost:3000/api/test?tags=tag1&tags=tag2&tags=tag3';
      const mockRequest = new NextRequest(url);
      const schema = z.object({
    ...
    it('should handle mixed array and single parameters', () => {
      const url = 'http://localhost:3000/api/test?name=John&tags=tag1&tags=tag2&age=30';
      const mockRequest = new NextRequest(url);
      const schema = z.object({
        name: z.string(),
        tags: z.array(z.string()),
    ...
    it('should throw ValidationError for Zod validation errors', () => {
      ...
      expect(() => validateQueryParams(mockRequest, schema)).toThrow(ValidationError);
    });
    ...
    it('should throw ValidationError for invalid URL format', () => {
      ...
      expect(() => validateQueryParams(mockRequest, schema)).toThrow(ValidationError);
      expect(() => validateQueryParams(mockRequest, schema)).toThrow('Invalid query parameters');
    });
    ...
    it('should handle encoded query parameters', () => {
      const url = 'http://localhost:3000/api/test?name=John%20Doe&message=Hello%20World';
      const mockRequest = new NextRequest(url);
      const schema = z.object({
        name: z.string(),
        message: z.string(),
  ...
  describe('validatePathParams', () => {
    ...
    it('should throw ValidationError for invalid parameters', () => {
      ...
      expect(() => validatePathParams(params, schema)).toThrow(ValidationError);
      expect(() => validatePathParams(params, schema)).toThrow('Path parameters validation failed');
    });
  ...
  describe('validatePathParam', () => {
    ...
    it('should handle null parameter', () => {
      ...
      expect(() => validatePathParam(param, schema)).toThrow(ValidationError);
      expect(() => validatePathParam(param, schema)).toThrow('Path parameter validation failed');
    });
  ...
  describe('validateRequest', () => {
    ...
    it('should propagate validation errors from params', async () => {
      ...
      await expect(validateFn(mockRequest, context)).rejects.toThrow(ValidationError);
    });
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle special characters in query parameters', () => {
      const url = 'http://localhost:3000/api/test?name=John%26Doe&symbol=%E2%9C%93';
      const mockRequest = new NextRequest(url);
      const schema = z.object({
        name: z.string(),
        symbol: z.string(),
