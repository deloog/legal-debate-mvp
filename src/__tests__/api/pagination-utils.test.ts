...
describe('Pagination Utils', () => {
  describe('createCursor', () => {
    ...
    it('should return limited items with cursor when more data exists', () => {
      ...
      // 验证游标包含正确的信息
      const parsed = JSON.parse(Buffer.from(result.nextCursor!, 'base64').toString());
  ...
  describe('parseCursor', () => {
    ...
    it('should handle complex nested cursor data', () => {
      ...
      const cursor = Buffer.from(JSON.stringify(complexData)).toString('base64');
  ...
  describe('estimateTotalCount', () => {
    ...
    it('should handle count function error', async () => {
      const countFunction = jest.fn().mockRejectedValue(new Error('Database error'));
      
      await expect(estimateTotalCount(countFunction)).rejects.toThrow('Database error');
    });
