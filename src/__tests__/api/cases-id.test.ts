...
import { createMockRequest, createTestResponse, assertions } from './test-utils';
...
describe('Cases ID API', () => {
  ...
  describe('GET /api/v1/cases/[id]', () => {
    it('should return case details', async () => {
      const request = createMockRequest(`http://localhost:3000/api/v1/cases/${mockCaseId}`);
      ...
      expect(testResponse.data.data.description).toBe('涉及买卖合同违约的纠纷案件');
    ...
    it('should handle invalid UUID', async () => {
      const request = createMockRequest('http://localhost:3000/api/v1/cases/invalid-id');
  ...
  describe('Error handling', () => {
    ...
    it('should console log for delete operations', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(`Deleting case with id: ${mockCaseId}`);
  ...
  describe('Integration tests', () => {
    it('should handle full CRUD flow', async () => {
      // GET
      const getRequest = createMockRequest(`http://localhost:3000/api/v1/cases/${mockCaseId}`);
      ...
      const putRequest = createMockRequest(`http://localhost:3000/api/v1/cases/${mockCaseId}`, {
        method: 'PUT',
        body: updateData,
      });
      ...
      // DELETE
      const deleteRequest = createMockRequest(`http://localhost:3000/api/v1/cases/${mockCaseId}`, {
        method: 'DELETE',
      });
      const deleteResponse = await DELETE(deleteRequest, { params: { id: mockCaseId } });
