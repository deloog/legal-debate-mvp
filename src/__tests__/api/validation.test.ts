...
describe('Validation Utils', () => {
  ...
  describe('validateUUID', () => {
    ...
    it('should use custom parameter name in error message', () => {
      ...
      expect(() => validateUUID(invalidUUID, 'CaseID')).toThrow('Invalid CaseID format');
    });
  ...
  describe('validateFileSize', () => {
    ...
    it('should throw error for file size exceeding limit', () => {
      ...
      expect(() => validateFileSize(oversizedFile, maxSize)).toThrow(ValidationError);
    });
  ...
  describe('validatePassword', () => {
    ...
    it('should maintain error order consistency', () => {
      ...
      expect(result.errors[0]).toBe('Password must be at least 8 characters long');
      expect(result.errors[1]).toBe('Password must contain at least one uppercase letter');
      expect(result.errors[2]).toBe('Password must contain at least one lowercase letter');
      expect(result.errors[3]).toBe('Password must contain at least one special character');
    });
  });
  ...
  describe('Integration Tests', () => {
    it('should handle validation pipeline for user creation', () => {
      ...
      expect(() => validateFileSize(userData.file.size, 1024 * 1024)).not.toThrow();
    });
    ...
    it('should handle validation failures in pipeline', () => {
      ...
      // File type validation
      const allowedTypes = ['image/jpeg', 'image/png', 'application/pdf'];
      expect(validateFileType(invalidUserData.file.type, allowedTypes)).toBe(false);

      // File size validation (1MB max)
      expect(() => validateFileSize(invalidUserData.file.size, 1024 * 1024)).toThrow(ValidationError);
    });
  });
});
