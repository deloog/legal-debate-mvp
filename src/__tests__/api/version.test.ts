...
describe('Version API', () => {
  describe('GET /api/version', () => {
    ...
    it('should include correct API version', async () => {
      ...
      expect(testResponse.data.data.api.description).toBe('RESTful API for Legal Debate System');
    });
    ...
    it('should include documentation information', async () => {
      ...
      expect(testResponse.data.data.documentation.postman).toBe('/api/docs/postman');
    });
    ...
    it('should include support information', async () => {
      ...
      expect(testResponse.data.data.support.contact).toBe('support@legaldebate.com');
      expect(testResponse.data.data.support.documentation).toBe('https://docs.legaldebate.com');
      expect(testResponse.data.data.support.issues).toBe('https://github.com/legaldebate/issues');
    });
  ...
  describe('HEAD /api/version', () => {
    it('should return 200 status without body', async () => {
      const request = createMockRequest('http://localhost:3000/api/version', { method: 'HEAD' });
