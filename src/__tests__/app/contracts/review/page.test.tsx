...
describe('合同审查页面', () => {
  ...
  describe('审查结果显示', () => {
    const mockUploadThenReview = (reviewData: object) => {
      (global.fetch as jest.Mock)
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ success: true, data: reviewData }),
        });
    };
    ...
    it('应该显示风险统计', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('总风险数')).toBeInTheDocument();
      });
    });
  });

  describe('风险项展示', () => {
    const triggerUploadAndReview = (reviewData: object) => {
      (global.fetch as jest.Mock)
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
    ...
    it('应该处理审查失败', async () => {
      (global.fetch as jest.Mock)
        ...
        .mockResolvedValueOnce({
          ...
          json: async () => ({
            success: false,
            error: { code: 'REVIEW_FAILED', message: '审查失败' },
          }),
        });
