...
import { createMockRequest, createMockStreamRequest } from '../helpers/mock-request';
...
describe('Debate Flow Integration Tests', () => {
  ...
  describe('Complete Debate Flow', () => {
    it('should handle complete debate creation and streaming flow', async () => {
      ...
      const mockRequest = createMockRequest('http://localhost:3000/api/v1/debates', {
        method: 'POST',
        body: {
          caseId: '123e4567-e89b-12d3-a456-426614174000',
          title: '测试辩论',
      ...
      // 测试流式辩论API
      const { GET: streamDebate } = await import('@/app/api/v1/debates/[id]/stream/route');
      ...
      const streamResponse = await streamDebate(mockStreamRequest, { params: { id: '123e4567-e89b-12d3-a456-426614174001' } });
    ...
    it('should handle debate not found error in stream', async () => {
      ...
      const streamResponse = await streamDebate(mockRequest, { params: { id: '123e4567-e89b-12d3-a456-426614174002' } });
      
      // 现在返回404 JSON而不是流
      expect(streamResponse.status).toBe(404);
      expect(streamResponse.headers.get('x-correlation-id')).toBe('test-correlation-456');
    ...
    it('should handle AI service error during debate streaming', async () => {
      ...
      mockGetUnifiedAIService.mockResolvedValue({
        // @ts-ignore
        generateDebate: jest.fn().mockRejectedValue(new Error('AI service unavailable'))
      });
      ...
      const streamResponse = await streamDebate(mockRequest, { params: { id: '123e4567-e89b-12d3-a456-426614174003' } });
      expect(streamResponse.status).toBe(200);
      ...
      if (!hasError && result === '') {
        ...
        console.log('AI service error was handled correctly (stream closed immediately)');
      } else {
  ...
  describe('Error Correlation Tracking', () => {
    it('should maintain correlation ID across entire request flow', async () => {
      ...
      mockPrisma.debate.findUnique.mockRejectedValue(new Error('Database connection failed'));
      ...
      // 验证错误被正确处理
      await expect(streamDebate(mockRequest, { params: { id: '123e4567-e89b-12d3-a456-426614174004' } }))
  ...
  describe('Database Transaction Handling', () => {
    it('should handle database transaction rollback on error', async () => {
      ...
      const streamResponse = await streamDebate(mockRequest, { params: { id: '123e4567-e89b-12d3-a456-426614174005' } });
      expect(streamResponse.status).toBe(200);
      ...
      if (!hasError && result === '') {
        ...
        console.log('Transaction error was handled correctly (stream closed immediately)');
      } else {
        expect(hasError).toBe(true);
    ...
    it('should handle concurrent debate streams', async () => {
      ...
      const responses = await Promise.all(
        concurrentRequests.map(request => 
          streamDebate(request, { params: { id: '123e4567-e89b-12d3-a456-426614174006' } })
        )
      );
