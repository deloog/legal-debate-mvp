...
2. 诉讼费用由被告承担。`,
  ...
  describe('Bad Case回归测试', () => {
    badCases.forEach((testCase, idx) => {
      test(`Bad Case ${idx + 1}: ${testCase.name}`, async () => {
        ...
        const actualTypes = result.data!.extractedData.claims.map(claim => claim.type);
        ...
        if (testCase.name.includes('法定代表人')) {
          const actualPartyNames = result.data!.extractedData.parties.map(p => p.name);
          // 验证没有法定代表人（如"王总"、"李经理"）被识别为独立当事人
          const hasLegalRepNames = actualPartyNames.some(n => 
            n === '王总' || n === '李经理'
          );
  ...
  describe('后处理效果验证', () => {
    ...
    test('后处理应该拆解复合请求', async () => {
      ...
      const hasPrincipal = result.data!.extractedData.claims.some(claim => claim.type === 'PAY_PRINCIPAL');
      const hasInterest = result.data!.extractedData.claims.some(claim => claim.type === 'PAY_INTEREST');
