...
describe('Bad Case: 诉讼请求识别测试', () => {
  ...
  describe('LITIGATION_COST补充测试', () => {
    it('Bad Case 1: 应该补充被遗漏的LITIGATION_COST', async () => {
      ...
      const hasLitigationCost = result.claims.some((c: any) => c.type === 'LITIGATION_COST');
  ...
  describe('复合请求拆解测试', () => {
    it('Bad Case 3: 拆解"本金及利息"为两个请求', async () => {
      ...
      const hasPrincipal = result.claims.some((c: any) => c.type === 'PAY_PRINCIPAL');
      const hasInterest = result.claims.some((c: any) => c.type === 'PAY_INTEREST');
    ...
    it('Bad Case 4: 拆解"货款及违约金"为两个请求', async () => {
      ...
      const hasPrincipal = result.claims.some((c: any) => c.type === 'PAY_PRINCIPAL');
      const hasPenalty = result.claims.some((c: any) => c.type === 'PAY_PENALTY');
  ...
  describe('PAY_PRINCIPAL补充测试', () => {
    ...
    it('Bad Case 6: 文本有"借款本金"应该识别PAY_PRINCIPAL', async () => {
      ...
      const principal = result.claims.find((c: any) => c.type === 'PAY_PRINCIPAL');
  ...
  describe('PAY_INTEREST识别测试', () => {
    it('Bad Case 9: 应该识别"支付利息"', async () => {
      ...
      const interest = result.claims.find((c: any) => c.type === 'PAY_INTEREST');
