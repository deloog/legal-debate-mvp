...
describe('TaskForm', () => {
  ...
  describe('表单提交测试', () => {
    ...
    it('应该在创建任务时显示保存中状态', async () => {
      const __mockFetch = jest.spyOn(global, 'fetch').mockImplementation(
        () =>
          new Promise(resolve => {
            setTimeout(() => {
              resolve({
    ...
    it('应该处理创建任务失败', async () => {
      const __mockFetch = jest.spyOn(global, 'fetch').mockResolvedValue({
        ok: false,
  ...
  describe('边界情况测试', () => {
    it('应该处理空描述', async () => {
      const __mockFetch = mockFetchSuccess();
      const onCancel = jest.fn();
      const onSuccess = jest.fn();
