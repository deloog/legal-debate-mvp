    ...
    it('should handle complete debate flow from start to finish', async () => {
      ...
      const legalBasis1 = await testPrisma.argument.create({
        data: {
          ...
          content: '根据《合同法》第107条，当事人一方不履行合同义务的，应当承担继续履行、采取补救措施或者赔偿损失等违约责任。',
      ...
      const plaintiffRebuttal = await testPrisma.argument.create({
        data: {
          ...
          side: 'PLAINTIFF' as const,
          content: '被告所称的质量问题没有第三方检测报告支持，且在收货时已签字确认货物完好。',
      ...
      const defendantSupporting = await testPrisma.argument.create({
        data: {
          ...
          side: 'DEFENDANT' as const,
          content: '附照片证据显示货物存在明显瑕疵，收货单上的签字仅确认收到货物，不表示确认质量。',
      ...
      const plaintiffConclusion = await testPrisma.argument.create({
        data: {
          ...
          side: 'PLAINTIFF' as const,
          content: '综上所述，被告未按合同约定支付货款构成违约，应立即支付货款并承担违约责任。',
      ...
      const defendantConclusion = await testPrisma.argument.create({
        data: {
          ...
          side: 'DEFENDANT' as const,
          content: '鉴于货物存在质量问题，原告要求支付货款缺乏事实和法律依据，请求驳回原告诉讼请求。',
          type: 'CONCLUSION' as const,
          aiProvider: 'zhipu',
          generationTime: 1900,
          confidence: 0.87,
      ...
      // 验证论点类型分布
      const mainPoints = round1Args.filter((arg: any) => arg.type === 'MAIN_POINT');
      const rebuttals = round2Args.filter((arg: any) => arg.type === 'REBUTTAL');
      const supporting = round2Args.filter((arg: any) => arg.type === 'SUPPORTING');
      const conclusions = round3Args.filter((arg: any) => arg.type === 'CONCLUSION');
      const legalBasis = round1Args.filter((arg: any) => arg.type === 'LEGAL_BASIS');
