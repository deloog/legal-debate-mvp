...
describe('Legal Reference Applicability Analysis', () => {
  ...
  describe('Applicability Reason Analysis', () => {
    it('should store detailed applicability reasons', async () => {
      ...
      expect(legalRef.applicabilityReason).toBe(refData.applicabilityReason);
      expect(legalRef.analysisResult).toEqual(refData.analysisResult);
      expect((legalRef.analysisResult as any).mainApplicability).toBe(
        '合同主体资格审查'
      );
    });
    ...
    it('should support multiple applicability reasons', async () => {
      ...
      expect(allRefs.map(ref => ref.applicabilityReason)).toEqual(
  ...
  describe('Analysis Result Structure', () => {
    it('should store complex analysis results', async () => {
      ...
      expect((legalRef.analysisResult as any).overallScore).toBe(
        complexAnalysis.overallScore
      );
      expect((legalRef.analysisResult as any).breakdown).toEqual(
        complexAnalysis.breakdown
      );
      expect((legalRef.analysisResult as any).riskFactors).toEqual(
        complexAnalysis.riskFactors
      );
    });
    ...
    it('should support different analysis providers', async () => {
      ...
      const humanRefs = allRefs.filter(ref =>
        ref.analyzedBy?.startsWith('HUMAN-')
      );
  ...
  describe('Analysis Timestamp Tracking', () => {
    it('should track analysis completion time', async () => {
      ...
      expect(legalRef.analyzedAt!.getTime()).toBeGreaterThan(
    ...
    it('should support analysis updates', async () => {
      ...
      expect(updatedRef.analyzedAt!.getTime()).toBeGreaterThan(
        originalAnalysis.getTime()
      );
    });
  });
  ...
  describe('Integration with Cases', () => {
    it('should analyze applicability for specific cases', async () => {
      const refData = {
        ...
        applicabilityReason:
          '专门适用于合同纠纷，包括合同订立、履行、变更、解除等各个环节',
        analysisResult: {
          caseTypes: ['合同纠纷', '民事纠纷'],
  ...
  describe('Quality Assurance', () => {
    ...
    it('should ensure analysis result integrity', async () => {
      ...
      expect((legalRef.analysisResult as any).factors).toEqual([
        '民事主体',
        '法律行为',
      ]);
