...
describe('Legal Reference Cache Management', () => {
  ...
  describe('Cache Hit Tracking', () => {
    ...
    it('should track access patterns', async () => {
      ...
      expect(finalRef?.lastAccessed).toEqual(accessTimes[accessTimes.length - 1]);
    });
  });
  ...
  describe('Cache Performance', () => {
    ...
    it('should maintain cache statistics', async () => {
      ...
      const lawstarStats = cacheStats.find(stat => stat.cacheSource === 'lawstar');
      const manualStats = cacheStats.find(stat => stat.cacheSource === 'manual');
