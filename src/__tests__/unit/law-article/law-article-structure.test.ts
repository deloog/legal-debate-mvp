    ...
    it('应该支持nullable字段为空', async () => {
      const article = await prisma.lawArticle.create({
        data: {
          ...
          issuingAuthority: '全国人民代表大会',
          searchableText:
            '为了惩罚犯罪保护人民根据宪法结合我国同犯罪作斗争的具体经验及实际情况制定本法',
        },
      });
  ...
  describe('数组字段验证', () => {
    ...
    it('应该支持relatedArticles数组', async () => {
      ...
      expect(article.relatedArticles).toEqual(
        expect.arrayContaining(relatedArticles)
      );
    });
  });
  ...
  describe('必需字段验证', () => {
    ...
    requiredFields.forEach(field => {
      it(`应该要求${field}字段`, async () => {
        ...
        await expect(prisma.lawArticle.create({ data })).rejects.toThrow();
      });
    });
