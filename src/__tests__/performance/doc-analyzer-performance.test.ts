...
describe('DocAnalyzer 性能测试', () => {
  ...
  describe('缓存性能测试', () => {
    ...
    it('缓存命中率应该>70%', async () => {
      ...
      // 第一次分析
      const context1 = buildAgentContext('perf-test-cache-hit-001', testFilePath);
      await agent.execute(context1);

      // 后续9次分析（应该命中缓存）
      for (let i = 2; i <= testCount; i++) {
        const startTime = Date.now();
        const context = buildAgentContext('perf-test-cache-hit-001', testFilePath);
        const result = await agent.execute(context);
        const processingTime = Date.now() - startTime;
      ...
      console.log(`缓存命中率: ${cacheHitRate.toFixed(2)}% (${cacheHits}/${testCount - 1})`);
  ...
  describe('并发处理性能测试', () => {
    it('应该能并发处理多个文档', async () => {
      ...
      const promises = Array.from({ length: concurrency }, (_, i) => {
        const context = buildAgentContext(`perf-test-concurrent-${i}`, testFilePath);
  ...
  describe('性能基准测试', () => {
    it('应该建立性能基准', async () => {
      ...
      for (let i = 0; i < runs; i++) {
        const startTime = Date.now();
        const context = buildAgentContext(`perf-test-benchmark-${i}`, testFilePath);
        const result = await agent.execute(context);
        const processingTime = Date.now() - startTime;
