...
const mockAIResponse = {
  swotAnalysis: {
    strengths: ["合同证据充分", "违约事实清晰", "法律依据明确"],
    weaknesses: ["部分款项未开票", "诉讼时效接近", "证据链存在瑕疵"],
    opportunities: ["可申请财产保全", "调解可能达成", "可追加利息"],
    threats: ["被告可能破产", "证据可能被质疑", "调解可能失败"],
  },
  strategies: [
    {
      strategy: "直接起诉策略",
      rationale: "基于违约事实，直接通过诉讼方式追偿",
      implementationSteps: ["准备起诉材料", "向法院提交起诉状", "参加庭审辩论"],
      expectedOutcome: "胜诉概率高，但耗时较长",
    },
    {
      strategy: "先行调解策略",
      rationale: "通过调解快速解决纠纷，节省时间和成本",
      implementationSteps: ["发出律师函", "协商调解方案", "签订调解协议"],
      expectedOutcome: "快速解决，但金额可能减少",
    },
    {
      strategy: "保全措施策略",
      rationale: "申请财产保全，确保胜诉后能执行",
      implementationSteps: ["收集财产线索", "提交保全申请", "缴纳保全费用"],
...
describe("StrategistAgent", () => {
  ...
  describe("AIStrategyGenerator", () => {
    ...
    it("应该抛出未初始化错误", async () => {
      await expect(generator.generateStrategy("测试", "测试")).rejects.toThrow(
        "AI服务未初始化",
      );
    });
    ...
    it("应该解析有效的AI响应", async () => {
      ...
      const response = await generator.generateStrategy("测试案件", "测试法条");
