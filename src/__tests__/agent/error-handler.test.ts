...
import { ErrorHandler, FallbackStrategyBuilder } from '@/lib/agent/coordinator/error-handler';
import type {
  FallbackStrategy,
  WorkflowStep,
  StepExecution,
...
describe('ErrorHandler', () => {
  ...
  describe('错误类型判断', () => {
    test('应该识别可重试的错误', () => {
      ...
      expect(errorHandler.isRetryableError(new Error('500 Internal Error'))).toBe(true);
      expect(errorHandler.isRetryableError(new Error('Rate limit exceeded'))).toBe(true);
    });

    test('应该识别不可重试的错误', () => {
      expect(errorHandler.isRetryableError(new Error('permission denied'))).toBe(false);
      expect(errorHandler.isRetryableError(new Error('invalid data'))).toBe(false);
    });

    test('应该识别应该中止的错误', () => {
      expect(errorHandler.shouldAbortWorkflow(new Error('permission denied'))).toBe(true);
      expect(errorHandler.shouldAbortWorkflow(new Error('authentication failed'))).toBe(true);
      expect(errorHandler.shouldAbortWorkflow(new Error('400 Bad Request'))).toBe(true);
    });

    test('应该在404错误时继续', () => {
      expect(errorHandler.shouldAbortWorkflow(new Error('404 Not Found'))).toBe(false);
    });
  });
});
