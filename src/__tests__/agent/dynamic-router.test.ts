...
describe('DynamicRouter', () => {
  ...
  beforeEach(() => {
    ...
    sampleWorkflow = {
      ...
      name: '测试工作流',
      steps: [
        { stepId: 'step1', agentType: 'ANALYZER' as any, name: '步骤1', required: true },
        { stepId: 'step2', agentType: 'STRATEGIST' as any, name: '步骤2', required: false },
        { stepId: 'step3', agentType: 'COORDINATOR' as any, name: '步骤3', required: false }
      ],
  ...
  describe('可路由步骤获取', () => {
    test('应该返回依赖已完成的步骤', () => {
      ...
      const routable = router.getRoutableSteps('step1', sampleWorkflow, context);
...
describe('WorkflowRouteBuilder', () => {
  ...
  describe('条件路由构建', () => {
    ...
    test('应该添加多个条件路由', () => {
      builder.addConditionalRoute('${input:type} === "A"', 'step2', '类型A路由');
      builder.addConditionalRoute('${input:type} === "B"', 'step3', '类型B路由');
