...
describe('AgentRegistry Enhanced Features', () => {
  ...
  describe('依赖注入支持的注册', () => {
    it('应该能够使用依赖注入注册Agent', () => {
      ...
      const success = registry.registerAgentWithDI(MockAgentClass, 'test-agent', {
        dependencies: { logger: 'logger', config: 'config' }
      });
    ...
    it('应该能够批量注册Agent（支持依赖注入）', () => {
      ...
      const agentConfigs = [
        { name: 'batch-agent-1', config: { dependencies: { logger: 'logger' } } },
        { name: 'batch-agent-2', config: { dependencies: { logger: 'logger', config: 'config' } } },
      ...
      const successCount = registry.registerAgentsWithDI(MockAgentClass, agentConfigs);
