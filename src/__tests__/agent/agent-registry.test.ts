...
interface Agent {
  ...
  validateInput?(input: any): { valid: boolean; errors?: string[]; warnings?: string[]; sanitizedData?: any; };
...
class SimpleAgentRegistry {
  private agents = new Map<string, { metadata: AgentMetadata; instance: Agent; registeredAt: number; }>();
  private eventListeners = new Map<string, Function[]>();

  async register(agent: Agent, metadata?: Partial<AgentMetadata>): Promise<void> {
...
describe('AgentRegistry', () => {
  ...
  describe('Registration', () => {
    it('should register an agent successfully', async () => {
      ...
      expect(registry.getMetadata('TestAgent')?.type).toBe(AgentType.DOC_ANALYZER);
    });
    ...
    it('should throw error when registering invalid agent', async () => {
      const invalidAgent = {
        ...
        execute: async () => ({ success: false, agentName: '', executionTime: 0 })
      } as Agent;
      
      await expect(registry.register(invalidAgent)).rejects.toThrow('Agent must have name and type');
    });
  });
  ...
  describe('Unregistration', () => {
    ...
    it('should throw error when unregistering non-existent agent', async () => {
      await expect(registry.unregister('NonExistent')).rejects.toThrow('Agent NonExistent not found');
    });
  });

  describe('Listing and Querying', () => {
    beforeEach(async () => {
      await registry.register(new MockAgent('DocAnalyzer', AgentType.DOC_ANALYZER));
      await registry.register(new MockAgent('Researcher', AgentType.RESEARCHER));
      await registry.register(new MockAgent('DocAnalyzer2', AgentType.DOC_ANALYZER));
    });
  ...
  describe('Status Management', () => {
    beforeEach(async () => {
      await registry.register(new MockAgent('TestAgent', AgentType.DOC_ANALYZER));
    });
