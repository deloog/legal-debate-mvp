...
import { FilterProcessor, FilterConfig } from '../../../../lib/agent/doc-analyzer/processors/filter-processor';
...
describe('FilterProcessor', () => {
  ...
  describe('OCR质量检查', () => {
    ...
    it('应该警告文本长度过短', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.warnings.some(w => w.includes('文本长度过短'))).toBe(true);
    });
    ...
    it('应该警告中文字符过少', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.warnings.some(w => w.includes('中文字符过少'))).toBe(true);
    });
  ...
  describe('文档类型分类', () => {
    ...
    it('应该正确识别行政文档', async () => {
      const text = '原告张三不服某局行政处罚决定，提起行政诉讼。' +
        '请求撤销该行政处罚决定。';
  ...
  describe('边界情况', () => {
    it('应该处理刚好达到最小要求的文本', async () => {
      const text = 'a'.repeat(15) + '张三李四王五赵六孙七周八吴九郑十诉' +
