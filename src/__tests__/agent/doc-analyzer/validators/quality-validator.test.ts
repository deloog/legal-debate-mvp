...
import type { ExtractedData, Party, Claim } from '@/lib/agent/doc-analyzer/core/types';
...
describe('QualityValidator', () => {
  ...
  describe('validate', () => {
    ...
    it('should info when litigation cost is missing', () => {
      ...
      const result = validator.validate(data, 0.9);
      expect(result.issues.some(i => i.message.includes('诉讼费用'))).toBe(true);
    ...
    it('should warn when amount is too large', () => {
      ...
      const result = validator.validate(data, 0.9);
      expect(result.issues.some(i => i.category === '金额' && i.severity === 'WARNING')).toBe(true);
    });
  ...
  describe('quickValidate', () => {
    ...
    it('should only include errors in result', () => {
      ...
      expect(quickResult.errors.every(e => e.severity === 'ERROR')).toBe(true);
    });
  });
});
