...
describe('RoleValidator', () => {
  ...
  describe('validate', () => {
    ...
    it('should fail when party name is empty', () => {
      ...
      expect(result.issues.some(i => i.message.includes('姓名为空'))).toBe(true);
    });

    it('should fail when party name is too short', () => {
      const parties: Party[] = [
        { type: 'plaintiff', name: '张' }
      ];
      const result = validator.validate(parties, mockValidClaims);
      expect(result.passed).toBe(false);
      expect(result.issues.some(i => i.message.includes('姓名过短'))).toBe(true);
    });
    ...
    it('should warn when party name is too long', () => {
      ...
      const result = validator.validate(parties, mockValidClaims);
      expect(result.passed).toBe(false);
      expect(result.issues.some(i => i.message.includes('姓名过长'))).toBe(true);
    });

    it('should warn when party name contains invalid pattern', () => {
      const parties: Party[] = [
        { type: 'plaintiff', name: '公司1' }
      ];
      const result = validator.validate(parties, mockValidClaims);
    ...
    it('should fail when plaintiff is missing with claims', () => {
      ...
      const result = validator.validate(parties, mockValidClaims);
      expect(result.passed).toBe(false);
      expect(result.issues.some(i => i.message.includes('缺少原告'))).toBe(true);
    });
    ...
    it('should fail when defendant is missing with claims', () => {
      ...
      const result = validator.validate(parties, mockValidClaims);
      expect(result.passed).toBe(false);
      expect(result.issues.some(i => i.message.includes('缺少被告'))).toBe(true);
    });
    ...
    it('should warn when plaintiff count is too high', () => {
      ...
      expect(result.issues.some(i => i.message.includes('原告数量过多'))).toBe(true);
    });
    ...
    it('should warn when defendant count is too high', () => {
      ...
      const result = validator.validate(parties, mockValidClaims);
      expect(result.issues.some(i => i.category === '角色完整性')).toBe(true);
      expect(result.issues.some(i => i.message.includes('被告数量过多'))).toBe(true);
    });
  ...
  describe('getSuggestions', () => {
    ...
    it('should not suggest when both roles exist', () => {
      const suggestions = validator.getSuggestions(mockValidParties, mockValidClaims);
