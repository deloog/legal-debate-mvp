...
describe("DisputeFocusExtractor - 准确率调试", () => {
  ...
  it("调试：逐个测试合同违约识别", async () => {
    ...
    for (const text of testCases) {
      ...
      const hasBreach = result.disputeFocuses.some(
      ...
      if (result.disputeFocuses.length > 0) {
        console.log(
          `  - Categories: ${result.disputeFocuses.map((f) => f.category).join(", ")}`,
        );
      }
    }
  });
  ...
  it("调试：逐个测试责任认定识别", async () => {
    ...
    for (const text of testCases) {
      ...
      const hasLiability = result.disputeFocuses.some(
