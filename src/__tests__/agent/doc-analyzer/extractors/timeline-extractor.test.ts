...
describe("TimelineExtractor", () => {
  ...
  describe("extractFromText", () => {
    ...
    it("应该提取履行开始事件", async () => {
      ...
      expect(result.events.length).toBeGreaterThan(0);
      const performanceEvent = result.events.find(
        (e) => e.eventType === "PERFORMANCE_START",
      );
    ...
    it("应该提取违约事件", async () => {
      ...
      expect(result.events.length).toBeGreaterThan(0);
      const breachEvent = result.events.find(
        (e) => e.eventType === "BREACH_OCCURRED",
      );
    ...
    it("应该提取起诉事件", async () => {
      ...
      expect(result.events.length).toBeGreaterThan(0);
      const lawsuitEvent = result.events.find(
        (e) => e.eventType === "LAWSUIT_FILED",
      );
    ...
    it("应该按时间排序事件", async () => {
      const text =
        "原告于2024年4月20日起诉，被告于2024年3月10日违约，双方于2024年1月15日签订合同。";
      const result = await extractor.extractFromText(text);
    ...
    it("应该生成正确的摘要", async () => {
      const text =
        "双方于2024年1月15日签订合同，2024年3月10日被告违约，2024年4月20日原告起诉。";
      const result = await extractor.extractFromText(text);
    ...
    it("应该识别多种日期格式", async () => {
      const text =
        "2024年1月15日签订合同，2024-02-01开始履行，2024年3月10日违约。";
      const result = await extractor.extractFromText(text);
  ...
  describe("extractTimelineFromText", () => {
    it("应该快速提取时间线", async () => {
      const events =
        await extractTimelineFromText("双方于2024年1月15日签订合同。");
    ...
    it("应该返回空数组", async () => {
      const events =
        await extractTimelineFromText("这是一段没有时间信息的文本");
  ...
  describe("时间线完整性验证", () => {
    it("应该提取完整的时间线", async () => {
      const text =
        "双方于2024年1月15日签订合同，2024年2月1日开始履行，2024年3月10日被告违约，2024年3月20日原告发函催告，2024年4月20日原告起诉。";
      const result = await extractor.extractFromText(text);
      ...
      expect(
        eventTypes.has("PERFORMANCE_START") ||
          eventTypes.has("BREACH_OCCURRED"),
      ).toBe(true);
  ...
  describe("多种日期格式测试", () => {
    const dateFormatTestCases = [
      {
        format: "YYYY年MM月DD日",
        text: "双方于2024年1月15日签订合同",
        expected: "2024-01-15",
      },
      {
        format: "YYYY-MM-DD",
        text: "双方于2024-01-15签订合同",
        expected: "2024-01-15",
      },
      {
        format: "YYYY/MM/DD",
    ...
    it("应该处理连续多个不同格式的日期", async () => {
      const text =
        "2024年1月15日签订合同，2024-02-01开始履行，2024/03/10发生违约。";
      const result = await extractor.extractFromText(text);
  ...
  describe("边界情况测试", () => {
    ...
    it("应该处理混合格式的日期文本", async () => {
      const text =
        "合同签订于2024年1月15日，但在2024-02-01变更，最后于2024/03/10终止。";
      const result = await extractor.extractFromText(text);
