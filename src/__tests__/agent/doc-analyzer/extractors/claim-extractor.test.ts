...
import { ClaimExtractor, createClaimExtractor, extractClaimsFromText } from '@/lib/agent/doc-analyzer/extractors/claim-extractor';
...
describe('ClaimExtractor', () => {
  ...
  describe('extractFromText', () => {
    ...
    it('应该处理复合请求', async () => {
      const result = await extractor.extractFromText('判令被告偿还本金5万元及利息');
    ...
    it('应该生成正确的摘要', async () => {
      const result = await extractor.extractFromText('偿还本金5万元，支付利息，诉讼费用由被告承担');
    ...
    it('应该过滤推断结果', async () => {
      const result = await extractor.extractFromText('本案费用由被告承担', { includeInferred: false });
  ...
  describe('normalizeClaims', () => {
    it('应该标准化诉讼请求', async () => {
      const claims: Claim[] = [
        { type: 'PAY_PRINCIPAL', content: '偿还本金', amount: 50000, currency: 'CNY', evidence: [], legalBasis: '' }
      ];
    ...
    it('应该设置默认货币', async () => {
      const claims: Claim[] = [
        { type: 'PAY_PRINCIPAL' as ClaimType, content: '偿还本金', amount: 50000, currency: 'CNY', evidence: [], legalBasis: '' }
      ];
