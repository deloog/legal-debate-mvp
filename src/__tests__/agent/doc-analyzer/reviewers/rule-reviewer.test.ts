...
describe('RuleReviewer', () => {
  ...
  describe('形式审查', () => {
    ...
    test('空诉讼请求应该报告ERROR', async () => {
      const data: ExtractedData = {
        parties: [],
        claims: [{ type: undefined, content: '', amount: undefined, currency: 'CNY' } as any]
      };
  ...
  describe('逻辑审查', () => {
    test('缺少原告应该报告WARNING', async () => {
      ...
      expect(result.issues.some(i => 
        i.category === 'LOGIC' && i.message.includes('缺少原告')
      )).toBe(true);
    });
    ...
    test('缺少被告应该报告WARNING', async () => {
      ...
      expect(result.issues.some(i => 
        i.category === 'LOGIC' && i.message.includes('缺少被告')
      )).toBe(true);
    });
    ...
    test('负数金额应该报告ERROR', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.issues.some(i => i.message.includes('负数金额'))).toBe(true);
    });
    ...
    test('多种货币单位应该报告WARNING', async () => {
      const data: ExtractedData = {
        parties: [],
        claims: [
          { type: 'PAY_PRINCIPAL', content: '测试1', amount: 1000, currency: 'CNY' },
          { type: 'PAY_INTEREST', content: '测试2', amount: 100, currency: 'USD' }
        ]
      };
      ...
      expect(result.issues.some(i => i.message.includes('多种货币单位'))).toBe(true);
    });
  });
  ...
  describe('完整性审查', () => {
    test('当事人数量不足应该报告WARNING', async () => {
      const data: ExtractedData = {
        parties: [],
        claims: [{ type: 'PAY_PRINCIPAL', content: '测试', amount: 1000, currency: 'CNY' }]
      };
    ...
    test('原文提及诉讼费用但未提取应该报告INFO', async () => {
      ...
      expect(result.issues.some(i => 
        i.category === 'COMPLETENESS' && i.severity === 'INFO'
      )).toBe(true);
    });
  });
