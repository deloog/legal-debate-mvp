...
export async function transitionRoundStatus(
  ...
  if (!isValidTransition(currentRound.status, newStatus)) {
    throw new Error(
      `无法从状态 ${currentRound.status} 转换到 ${newStatus}`
    );
  }
...
async function updateDebateProgress(debateId: string) {
  ...
  const inProgressRounds = rounds.filter(r => r.status === RoundStatus.IN_PROGRESS).length;
...
export async function startRound(
  ...
  return transitionRoundStatus(roundId, RoundStatus.IN_PROGRESS, { userId, reason });
}
...
export async function completeRound(
  ...
  return transitionRoundStatus(roundId, RoundStatus.COMPLETED, { userId, reason });
}
...
export async function retryRound(
  ...
  return transitionRoundStatus(roundId, RoundStatus.PENDING, { userId, reason });
}
