...
export class DocAnalyzerMonitor {
  ...
  getQualityTrend(period: 'hour' | 'day' | 'week' = 'day'): TrendAnalysis {
    ...
    const averageQualityScore = this.calculateAverage(periodMetrics.map(m => m.qualityScore));
    const averageProcessingTime = this.calculateAverage(periodMetrics.map(m => m.processingTime));
    const successRate = periodMetrics.filter(m => m.validationResults.isValid).length / periodMetrics.length;
   ...
   * 分析问题分布
   */
  private analyzeIssueDistribution(metrics: QualityMetrics[]): { [key: string]: number } {
  ...
  private calculateQualityTrend(metrics: QualityMetrics[]): 'improving' | 'stable' | 'declining' {
    ...
    const secondAvg = this.calculateAverage(secondHalf.map(m => m.qualityScore));
  ...
  private checkAlerts(metric: QualityMetrics): void {
    ...
    if (metric.qualityScore < this.config.minQualityScore) {
      this.createAlert('warning', `质量评分过低: ${(metric.qualityScore * 100).toFixed(1)}%`, {
        metric,
    ...
    // 验证失败
    if (!metric.validationResults.isValid && metric.validationResults.issues.length > 0) {
      this.createAlert('error', `验证失败: ${metric.validationResults.issues.join(', ')}`, {
        metric
      });
    }
  }
   ...
   * 创建告警
   */
  private createAlert(level: 'warning' | 'error' | 'critical', message: string, details?: any): void {
    const alert: Alert = {
      timestamp: Date.now(),
      level,
      message,
   ...
   * 发送告警到指定渠道
   */
  private sendAlert(channel: 'console' | 'log' | 'external', alert: Alert): void {
    switch (channel) {
      case 'console':
        console.warn(`[DocAnalyzer Alert] [${alert.level.toUpperCase()}] ${alert.message}`, alert.details || '');
        break;
      case 'log':
        // 实际项目中可以写入日志文件
        break;
  ...
  getStats(): {
    ...
    return {
      ...
      averageQualityScore: this.calculateAverage(this.metrics.map(m => m.qualityScore)),
      averageProcessingTime: this.calculateAverage(this.metrics.map(m => m.processingTime))
    };
  }
}
 ...
 * 便捷函数：获取质量趋势
 */
export function getDocAnalyzerTrend(period?: 'hour' | 'day' | 'week'): TrendAnalysis {
