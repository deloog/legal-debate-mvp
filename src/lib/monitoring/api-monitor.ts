...
export class APIMonitor {
   ...
   * 记录API请求指标
   */
  static async logRequest(metrics: Omit<APIMetrics, 'timestamp'>): Promise<void> {
    try {
   ...
   * 记录数据库操作指标
   */
  static async logDatabaseOperation(metrics: Omit<DatabaseMetrics, 'timestamp'>): Promise<void> {
    try {
      await prisma.aIInteraction.create({
        data: {
   ...
   * 记录AI服务指标
   */
  static async logAIOperation(metrics: Omit<AIMetrics, 'timestamp'>): Promise<void> {
    try {
      await prisma.aIInteraction.create({
        data: {
          type: 'AI_OPERATION',
   ...
   * 记录业务事件
   */
  static async logBusinessEvent(metrics: Omit<BusinessMetrics, 'timestamp'>): Promise<void> {
    try {
      await prisma.aIInteraction.create({
        data: {
          type: 'BUSINESS_EVENT',
  ...
  static async getAPIStats(
    ...
    // 统计端点性能
    const endpointStats = interactions.reduce((acc, interaction) => {
  ...
  static async getAIStats(
    ...
    const averageDuration = interactions.length > 0
      ? interactions.reduce((sum, i) => sum + (i.duration || 0), 0) / interactions.length
      : 0;

    const totalTokens = interactions.reduce((sum, i) => sum + (i.tokensUsed || 0), 0);
    ...
    const providerStatsArray = Object.entries(providerStats).map(([provider, stats]) => ({
      provider,
      count: stats.count,
  ...
  static async getBusinessStats(
    ...
    // 统计事件类型
    const eventTypeStats = interactions.reduce((acc, interaction) => {
    ...
    // 统计实体类型
    const entityTypeStats = interactions.reduce((acc, interaction) => {
    ...
    return {
      totalEvents,
      eventTypeStats: Object.entries(eventTypeStats).map(([eventType, count]) => ({
        eventType,
        count,
      })),
      entityTypeStats: Object.entries(entityTypeStats).map(([entityType, count]) => ({
        entityType,
        count,
      })),
    };
    ...
    try {
      ...
      await prisma.aIInteraction.deleteMany({
        where: {
          ...
          type: {
            in: ['API_REQUEST', 'DATABASE_OPERATION', 'AI_OPERATION', 'BUSINESS_EVENT'],
          },
        },
      });
...
export function createPerformanceTracker(endpoint: string, method: string) {
  ...
  return {
    requestId,
    endTime: async (statusCode: number, userId?: string, userAgent?: string, ip?: string, error?: string) => {
 ...
 * AI服务调用监控装饰器
 */
export function monitorAICall(provider: string, operation: string, model?: string) {
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    const method = descriptor.value;
