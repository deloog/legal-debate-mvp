...
export class AIMonitor {
  ...
  private initializeAlertThresholds(): void {
    if (this.config.alertThresholds) {
      this.alertThresholds.set('responseTime', this.config.alertThresholds.responseTime);
      this.alertThresholds.set('errorRate', this.config.alertThresholds.errorRate);
      this.alertThresholds.set('rateLimitHits', this.config.alertThresholds.rateLimitHits);
      this.alertThresholds.set('queueLength', this.config.alertThresholds.queueLength);
    }
  }
  ...
  public recordFallbackActivation(provider: AIProvider, strategy: string): void {
  ...
  public recordHealthCheck(provider: AIProvider, healthy: boolean, responseTime: number): void {
  ...
  private updateHealthStatus(
    ...
    // 根据响应时间和错误率更新健康状态
    const isHealthy = this.evaluateHealth(provider, success, responseTime, errorType);
  ...
  private evaluateHealth(
    ...
    // 响应时间过长，标记为不健康
    const responseTimeThreshold = this.alertThresholds.get('responseTime') || 10000; // 10秒
    if (responseTime > responseTimeThreshold) {
      return false;
    }

    // 特定错误类型，标记为不健康
    const criticalErrors = ['authentication_error', 'permission_error', 'insufficient_quota'];
  ...
  private triggerAlert(type: string, provider: AIProvider, data: Record<string, unknown>): void {
    const alert = {
      type,
      provider,
      timestamp: Date.now(),
  ...
  private logEvent(type: MonitorEventType, provider?: AIProvider, model?: string, data?: Record<string, unknown>): void {
  ...
  private outputLog(event: MonitorEvent): void {
    ...
    const message = `[${new Date(event.timestamp).toISOString()}] ${event.type.toUpperCase()}` +
  ...
  private collectMetrics(): void {
    ...
    providers.forEach(provider => {
      const recentMetrics = this.getRecentMetrics(provider, this.config.metricsInterval);
  ...
  private getRecentMetrics(provider: AIProvider, timeWindow: number): PerformanceMetrics[] {
  ...
  public getServiceStatus(): ServiceStatus {
    const totalRequests = Array.from(this.metrics.values())
      .reduce((sum, metricsList) => sum + metricsList.length, 0);
    
    const totalErrors = Array.from(this.metrics.values())
      .reduce((sum, metricsList) => 
        sum + metricsList.filter(m => !m.success).length, 0
      );
  ...
  public getMetrics(provider?: AIProvider, model?: string, timeWindow?: number): PerformanceMetrics[] {
  ...
  public getEvents(eventType?: MonitorEventType, timeWindow?: number): MonitorEvent[] {
  ...
  public getHealthStatus(provider?: AIProvider): HealthStatus | Map<AIProvider, HealthStatus> {
    if (provider) {
  ...
  private getProviderReport(metrics: PerformanceMetrics[]): Record<string, any> {
...
export class MonitorFactory {
  ...
  public static getInstance(name: string = 'default', config?: MonitorConfig): AIMonitor {
  ...
  public static createCustomInstance(name: string, config: MonitorConfig): AIMonitor {
