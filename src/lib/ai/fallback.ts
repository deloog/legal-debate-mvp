...
export class FallbackManager {
  ...
  public async handleFailure(
    ...
    try {
      ...
      for (const strategy of strategies) {
        ...
        try {
          const result = await this.executeStrategy(strategy, originalRequest, providers);
          ...
          console.warn(`Fallback strategy ${strategy.action} failed:`, strategyError);
          continue;
        }
      }
  ...
  private getStrategiesForCondition(condition: FallbackCondition): FallbackStrategy[] {
  ...
  private async useCache(originalRequest: AIRequestConfig): Promise<AIResponse | null> {
  ...
  private async simplifiedRequest(originalRequest: AIRequestConfig): Promise<AIResponse | null> {
    ...
    try {
      ...
      const simplifiedConfig: AIRequestConfig = {
        ...
        temperature: originalRequest.temperature ? Math.min(originalRequest.temperature, 0.5) : 0.3,
      };

      // 如果启用了简化提示
      if (this.config.simplifiedMode.simplifiedPrompts) {
        simplifiedConfig.messages = this.simplifyMessages(originalRequest.messages);
      }
  ...
  private async localProcessing(originalRequest: AIRequestConfig): Promise<AIResponse | null> {
    ...
    try {
      ...
      if (this.config.localProcessing.capabilities.includes('text_generation')) {
        responseContent = this.generateLocalResponse(userContent);
      } else if (this.config.localProcessing.capabilities.includes('template_response')) {
        responseContent = this.getTemplateResponse(userContent);
      } else {
        responseContent = 'I apologize, but I am currently experiencing technical difficulties. Please try again later.';
      }
  ...
  private async returnError(originalRequest: AIRequestConfig): Promise<AIResponse | null> {
    console.log('Returning error response as fallback');
    
    const errorMessage = 'I apologize, but I am currently unable to process your request due to service limitations. Please try again later.';
  ...
  public getFallbackStats(timeWindow?: number): FallbackStats {
    ...
    const relevantEvents = this.fallbackHistory.filter(e => e.timestamp > cutoffTime);
    ...
    return {
      ...
      strategyStats,
      errorTypeStats,
      averageResolutionTime: this.calculateAverageResolutionTime(relevantEvents.filter(e => e.success)),
    };
  }

  private calculateAverageResolutionTime(successfulEvents: FallbackEvent[]): number {
...
export class FallbackManagerFactory {
  ...
  public static getInstance(name: string = 'default', config?: FallbackConfig, cacheManager?: any): FallbackManager {
  ...
  public static createCustomInstance(name: string, config: FallbackConfig, cacheManager?: any): FallbackManager {
