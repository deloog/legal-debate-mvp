...
export class LawsuitRequestClassifier {
  private readonly classificationRules: Map<LawsuitRequestType, ClassificationRule> = new Map();
   ...
   * 分类诉讼请求
   */
  async classifyLawsuitRequests(requestText: string): Promise<ClassificationResult> {
    ...
    // 第四步：验证和清理
    const validation = this.validateClassification(processedClaims, requestText);
    ...
    return {
      ...
      hasCompoundClaims: processedClaims.some(c => c.subClaims && c.subClaims.length > 0),
   ...
   * 分类单个请求
   */
  private async classifySingleClaim(claimText: string): Promise<ClassifiedClaim | null> {
    const matchedTypes: { type: LawsuitRequestType; score: number; keywords: string[]; patterns: string[] }[] = [];
    ...
    // 提取金额
    const amountResults = await this.amountExtractor.extractWithPrecision(claimText);
    ...
    for (let i = 0; i < claims.length; i++) {
      ...
      if (this.shouldCombineClaims(current, next)) {
        ...
        const compoundClaim: ClassifiedClaim = {
          ...
          matchedKeywords: [...current.matchedKeywords, ...next.matchedKeywords],
          matchedPatterns: [...current.matchedPatterns, ...next.matchedPatterns],
   ...
   * 判断是否应该合并请求
   */
  private shouldCombineClaims(claim1: ClassifiedClaim, claim2: ClassifiedClaim): boolean {
    // 同一原文中的连续请求
    if (!claim1.originalText.includes('；') && !claim2.originalText.includes('；')) {
      return false;
    }
    ...
    return moneyTypes.includes(claim1.classifiedType) && moneyTypes.includes(claim2.classifiedType);
  }

  /**
   * 验证分类结果
   */
  private validateClassification(claims: ClassifiedClaim[], originalText: string): { unclassifiedText: string[]; warnings: string[] } {
    ...
    for (const claim of claims) {
      typeCounts.set(claim.classifiedType, (typeCounts.get(claim.classifiedType) || 0) + 1);
    }
