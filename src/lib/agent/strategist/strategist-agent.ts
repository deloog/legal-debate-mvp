...
export class StrategistAgent extends BaseAgent {
  ...
  public readonly description = '策略制定智能体，实现AI生成+规则验证+风险评估三层架构';
  ...
  protected async executeLogic(context: AgentContext): Promise<StrategyOutput> {
    ...
    try {
      ...
      if (!ruleValidation.valid && this.config.enableStrictValidation !== false) {
        logger.warn('策略验证未通过', {
   ...
   */
  public validateInput(input: StrategyInput): { valid: boolean; errors?: string[] } {
    const errors: string[] = [];
    ...
    if (!input.caseInfo) {
      ...
      if (!input.caseInfo.caseType || typeof input.caseInfo.caseType !== 'string') {
        errors.push('案件类型必须是非空字符串');
      }
      if (!Array.isArray(input.caseInfo.parties) || input.caseInfo.parties.length === 0) {
        errors.push('当事人信息必须是非空数组');
      }
      if (!Array.isArray(input.caseInfo.claims) || input.caseInfo.claims.length === 0) {
        errors.push('诉讼请求必须是非空数组');
      }
      if (!Array.isArray(input.caseInfo.facts) || input.caseInfo.facts.length === 0) {
