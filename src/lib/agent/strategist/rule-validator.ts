...
export class RuleValidator {
  ...
  validate(strategy: AIStrategyResponse): RuleValidationResult {
    ...
    // 验证策略建议
    this.validateStrategies(strategy.strategies, violations, warnings, suggestions);
  ...
  private validateStrategies(
    ...
    for (let i = 0; i < strategies.length; i++) {
      ...
      if (!strategy.expectedOutcome || typeof strategy.expectedOutcome !== 'string') {
  ...
  private validateRisks(
    ...
    for (let i = 0; i < risks.length; i++) {
      ...
      if (!risk.impact || typeof risk.impact !== 'string') {
        ...
        !['low', 'medium', 'high'].includes(risk.impact)
      ) {
        violations.push(
          `第${i + 1}个风险的impact值无效，应为low/medium/high`
        );
      }
  ...
  private generateSuggestions(
    ...
    // SWOT分析改进建议
    if (
      strategy.swotAnalysis.strengths.length === this.config.minSWOTItems
    ) {
      suggestions.push('建议补充更多优势分析，以全面评估案件');
    }

    if (
      strategy.swotAnalysis.weaknesses.length === this.config.minSWOTItems
    ) {
