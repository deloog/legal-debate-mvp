...
export class SecureFileUtils {
  ...
  static validateFilePath(filePath: string): void {
    ...
    try {
      ...
      if (stats.size > this.MAX_FILE_SIZE) {
        throw new SecurityError(`文件过大: ${stats.size} bytes (最大允许: ${this.MAX_FILE_SIZE} bytes)`, {
          filePath: normalizedPath,
          fileSize: stats.size,
      ...
      throw new SecurityError('无法读取文件信息', { filePath: normalizedPath, error });
    }
  }
  ...
  static async readFileSecurely(filePath: string): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(filePath, (err, data) => {
        if (err) {
          reject(new SecurityError(`读取文件失败: ${err.message}`, { filePath, error: err }));
        } else {
          resolve(data);
        }
      });
   ...
   * 安全地读取文本文件
   */
  static async readTextFileSecurely(filePath: string, encoding: BufferEncoding = 'utf-8'): Promise<string> {
  ...
  static async executeCommandSecurely(
    ...
    if (!allowedCommands.includes(command)) {
      throw new SecurityError(`不允许执行的命令: ${command}`, { command, allowedCommands });
    }
    ...
    try {
      ...
      throw new SecurityError(`命令执行失败: ${error instanceof Error ? error.message : String(error)}`, {
        command,
        args,
        error
      });
  ...
  static async getFileSizeSecurely(filePath: string): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      fs.stat(filePath, (err, stats) => {
        if (err) {
          reject(new SecurityError(`获取文件信息失败: ${err.message}`, { filePath, error: err }));
        } else {
          resolve(stats.size);
        }
      });
