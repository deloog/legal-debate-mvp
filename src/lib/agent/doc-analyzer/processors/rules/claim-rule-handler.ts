...
export class ClaimRuleHandler {
  ...
  private addLitigationCost(
    ...
    if (
      !hasLitigationCost &&
      this.matchesPatterns(text, POST_PROCESSING_RULES.LITIGATION_COST_PATTERNS)
    ) {
      claims.push({
  ...
  private addPrincipalClaim(
    ...
    if (
      !claims.some(c => c.type === 'PAY_PRINCIPAL') &&
  ...
  private addInterestClaim(
    ...
    if (
      !claims.some(c => c.type === 'PAY_INTEREST') &&
  ...
  private addPenaltyClaim(
    ...
    if (
      !claims.some(c => c.type === 'PAY_PENALTY') &&
  ...
  private decomposeCompoundClaims(
    ...
    for (const pattern of POST_PROCESSING_RULES.COMPOUND_PATTERNS) {
      if (pattern.regex.test(text)) {
        logger.info('识别到复合请求模式，强制拆解', {
          pattern: pattern.regex.source,
        });

        const compoundClaim = claims.find(
          c => c.content.length > 20 && pattern.regex.test(c.content)
        );
