...
export class TextExtractor {
  ...
  private async extractDOCText(filePath: string): Promise<string> {
    try {
      ...
      // 首先尝试使用antiword（安全版本）
      try {
        return await SecureFileUtils.executeCommandSecurely('antiword', [filePath]);
      } catch (antiwordError) {
        logger.warn('antiword不可用，尝试使用textract', { filePath, error: antiwordError });
   ...
   * 智能文本分块（按句子边界）
   */
  splitTextSmart(text: string, maxChunkSize: number): Array<{ text: string; start: number; end: number }> {
    ...
    while (currentPos < text.length) {
      ...
      if (endPos < text.length) {
        ...
        while ((match = sentenceEnders.exec(text.substring(currentPos, endPos))) !== null) {
