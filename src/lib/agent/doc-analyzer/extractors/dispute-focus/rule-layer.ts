...
export function ruleMatchLayer(
  ...
  for (const [category, patterns] of patternsMap) {
    console.log(`[规则匹配层] 处理类别: ${category}, 模式数量: ${patterns.length}`);
    for (const pattern of patterns) {
      const matches = text.matchAll(pattern);
      for (const match of matches) {
        // 检查是否已被AI提取层覆盖
        const isAlreadyExtracted = aiExtracted?.some(
          aiFocus => isSimilarFocus(aiFocus, match[0], category)
        );
...
function isSimilarFocus(
  ...
  const overlap = aiDesc.substring(0, intersection) === matchedLower.substring(0, intersection);

  // 如果核心争议点相同，也认为是相似的
  const coreIssueMatch = aiFocus.coreIssue === matchedText ||
 ...
 * 提取原告观点
 */
function extractPositionA(text: string, matchedText: string, matchIndex: number): string {
  const patterns = [
    /原告认为|原告主张|原告称/gi,
    /原告方面/gi,
    /起诉方认为/gi
 ...
 * 提取被告观点
 */
function extractPositionB(text: string, matchedText: string, matchIndex: number): string {
  const patterns = [
    /被告认为|被告主张|被告称/gi,
    /被告方面/gi,
    /答辩方认为/gi,
 ...
 * 提取法律依据
 */
function extractLegalBasis(matchedText: string, fullText: string, matchIndex: number): string | undefined {
  const patterns = [
    /依据\s*《[^》]+》/gi,
    /根据\s*《[^》]+》/gi,
    /按照\s*《[^》]+》/gi,
