...
export class ClaimExtractor {
  ...
  async extractFromText(
    ...
    if (options.decomposeCompound !== false) {
      const { decomposedClaims, decomposedCount } = this.decomposeCompoundClaims(
        claims,
        text
      );
  ...
  private tryDecomposeClaim(claim: Claim, fullText: string): Claim[] {
    ...
    for (const pattern of this.compoundPatterns) {
      if (pattern.originalText && claim.content.includes(pattern.originalText)) {
      ...
      if (pattern.types.some(t => t === claim.type)) {
        for (const type of pattern.types) {
          if (pattern.types.includes(type) && !decomposed.some(c => c.type === type)) {
            decomposed.push({
              type,
  ...
  private addMissingClaimTypes(claims: Claim[], fullText: string): Claim[] {
    ...
    // 强制补充LITIGATION_COST
    if (!existingTypes.has('LITIGATION_COST') && this.likelyHasLitigationCost(fullText)) {
      // 修复Bad Case 2: 确保内容包含"诉讼费用"字符串
      const hasExplicitMention = /诉讼费用|本案.*诉讼费|全部诉讼费用/.test(fullText);
      added.push({
    ...
    // 强制补充PAY_PRINCIPAL
    if (!existingTypes.has('PAY_PRINCIPAL') && this.likelyHasPrincipal(fullText)) {
      added.push({
        type: 'PAY_PRINCIPAL',
        content: '偿还本金（从文本推断）',
        amount: undefined,
    ...
    // 强制补充PAY_INTEREST
    if (!existingTypes.has('PAY_INTEREST') && this.likelyHasInterest(fullText)) {
      added.push({
        type: 'PAY_INTEREST',
        content: '支付利息（从文本推断）',
        amount: undefined,
  ...
  private initializeClaimPatterns(): Map<ClaimType, RegExp[]> {
    ...
    patterns.set('PERFORMANCE', [
      /履行义务|继续履行/gi,
      /履行合同/gi
    ]);

    patterns.set('TERMINATION', [
      /解除合同|终止合同/gi,
      /解除.*合同/gi
    ]);
